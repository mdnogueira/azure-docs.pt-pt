---
title: Introdução ao File Storage do Azure no Windows | Microsoft Docs
...
## Descrição geral
O File Storage do Azure é um serviço que oferece partilhas de ficheiros na nuvem utilizando o [Protocolo SMB (Server Message Block)](https://msdn.microsoft.com/library/windows/desktop/aa365233.aspx) padrão. O SMB 2.1 e o SMB 3.0 são suportados. Com o File Storage do Azure, é possível migrar aplicações antigas que se baseiam em partilhas de ficheiros para o Azure rapidamente e sem reescritas dispendiosas. As aplicações em execução em Virtual Machines ou Cloud Services do Azure ou a partir de clientes no local podem montar uma partilha de ficheiros na nuvem, tal como uma aplicação de ambiente de trabalho monta uma partilha SMB típica. Em seguida, quaisquer componentes da aplicação podem montar e aceder à partilha do File Storage em simultâneo.
...
Para obter informações sobre metas de desempenho e escalabilidade do File Storage, consulte o artigo [Metas de desempenho e escalabilidade do File Storage](storage-scalability-targets.md#scalability-targets-for-blobs-queues-tables-and-files).

[!INCLUDE [storage-dotnet-client-library-version-include](../../includes/storage-dotnet-client-library-version-include.md)]

[!INCLUDE [storage-file-concepts-include](../../includes/storage-file-concepts-include.md)]

## Vídeo: utilizar o File Storage do Azure com o Windows
...
Este tutorial de introdução demonstra as noções básicas da utilização do File Storage do Microsoft Azure. Neste tutorial, iremos:
...
* Utilizar a Biblioteca de Clientes do Storage do Azure para .NET para aceder à partilha de ficheiros a partir de uma aplicação no local. Criar uma aplicação de consola e realizar estas ações com a partilha de ficheiros:
  * Escrever o conteúdo de um ficheiro na partilha da janela de consola.
  ...
  * Copiar um ficheiro para outro ficheiro na mesma conta do Storage.
  * Copiar um ficheiro para um blob na mesma conta do Storage.
* Utilizar as Métricas do Storage do Azure para a resolução de problemas

O File Storage é agora suportado para todas as contas do Storage, pelo que pode utilizar uma conta do Storage existente ou pode criar uma nova conta do Storage. Consulte o artigo [Como criar uma conta do Storage](storage-create-storage-account.md#create-a-storage-account) para obter informações sobre como criar uma nova conta do Storage.

## Utilizar o Portal do Azure para gerir uma partilha de ficheiros
...
2. No menu de navegação, clique em **Contas do Storage** ou **Contas do Storage (clássicas)**.
...
3. Escolha a sua conta do Storage.
...
### Carregar e transferir os ficheiros
...
2. Clique em **Ligar** para obter a linha de comandos para montar a partilha de ficheiros a partir do Windows.
   ...
   > [!TIP]
   > Para localizar a chave de acesso da conta do Storage para a montagem, clique nas **Definições** da sua conta do Storage e, em seguida, clique em **Chaves de acesso**.
   > 
   > 
    ...
    ![Captura de ecrã que mostra como encontrar a chave de acesso da conta do Storage](./media/storage-dotnet-how-to-use-files/files-manage-5.png)

## Utilizar o PowerShell para gerir uma partilha de ficheiros
Em alternativa, pode utilizar o Azure PowerShell para criar e gerir partilhas de ficheiros.

### Instalar os cmdlets do PowerShell para o Storage do Azure
Para preparar para utilizar o PowerShell, transfira e instale os cmdlets do Azure PowerShell. Consulte o artigo [Como instalar e configurar o Azure PowerShell](../powershell-install-configure.md) para obter as instruções de instalação e do ponto de instalação.
...
### Criar um contexto para a conta do Storage e a chave
Agora, crie o contexto da conta do Storage. O contexto contém a chave de conta e o nome da conta do Storage. Para obter instruções sobre a cópia da chave de conta a partir do [Portal do Azure](https://portal.azure.com), consulte o artigo [Ver e copiar teclas de acesso de armazenamento](storage-create-storage-account.md#view-and-copy-storage-access-keys).

Substitua `storage-account-name` e `storage-account-key` pelo nome da conta do Storage e a chave no exemplo seguinte.

    # create a context for account and key
    $ctx=New-AzureStorageContext storage-account-name storage-account-key

### Criar uma nova partilha de ficheiros
Em seguida, crie a nova partilha, com o nome `logs`.

    # create a new share
    $s = New-AzureStorageShare logs -Context $ctx

Tem agora uma partilha de ficheiros no File Storage. Em seguida, vamos adicionar um diretório e um ficheiro.

> [!IMPORTANT]
> O nome da partilha de ficheiros tem de ser todo em minúsculas. Para obter detalhes completos sobre a nomenclatura de partilhas de ficheiros e ficheiros, consulte o artigo [Nomenclatura e Referência de Partilhas, Diretórios, Ficheiros e Metadados](https://msdn.microsoft.com/library/azure/dn167011.aspx).
> 
> 

### Criar um diretório na partilha de ficheiros
Em seguida, crie um diretório na partilha. No exemplo seguinte, o diretório chama-se `CustomLogs`.

    # create a directory in the share
    New-AzureStorageDirectory -Share $s -Path CustomLogs

### Carregar um ficheiro local para o diretório
Agora, carregue um ficheiro local para o diretório. O exemplo seguinte carrega um ficheiro de `C:\temp\Log1.txt`. Edite o caminho do ficheiro para que aponte para um ficheiro válido no seu computador local.

    # upload a local file to the new directory
    Set-AzureStorageFileContent -Share $s -Source C:\temp\Log1.txt -Path CustomLogs

### Listar os ficheiros no diretório
...
### Copiar ficheiros
A partir da versão 0.9.7 do Azure PowerShell, pode copiar um ficheiro para outro ficheiro, um ficheiro para um blob ou um blob para um ficheiro. Abaixo, demonstramos como realizar estas operações de cópia utilizando cmdlets do PowerShell.
...
### Montar a partilha de ficheiros a partir de um computador virtual do Azure a executar o Windows
Para demonstrar como montar uma partilha de ficheiros do Azure, vamos agora criar um computador virtual do Azure a executar o Windows e aceder remotamente ao mesmo para montar a partilha.

1. Primeiro, crie um novo computador virtual do Azure, seguindo as instruções em [Criar um computador virtual do Windows no Portal do Azure](../virtual-machines/virtual-machines-windows-hero-tutorial.md).
2. Em seguida, aceda remotamente ao computador virtual ao seguir as instruções em [Iniciar sessão num computador virtual do Windows através do Portal do Azure](../virtual-machines/virtual-machines-windows-connect-logon.md).
3. Abra uma janela do PowerShell no computador virtual.

### Manter as suas credenciais da conta do Storage para o computador virtual
Antes da montagem para a partilha de ficheiros, primeiro mantenha as credenciais da conta do Storage no computador virtual. Este passo permite ao Windows restabelecer automaticamente ligação à partilha de ficheiros quando o computador virtual reiniciar. Para manter as suas credenciais de conta, execute o comando `cmdkey` a partir da janela do PowerShell no computador virtual. Substitua `<storage-account-name>` pelo nome da sua conta do Storage e `<storage-account-key>` pela chave da conta do Storage.

    cmdkey /add:<storage-account-name>.file.core.windows.net /user:<storage-account-name> /pass:<storage-account-key>

Em seguida, o Windows irá restabelecer ligação à partilha de ficheiros quando o computador virtual reiniciar. Pode verificar se a ligação à partilha foi restabelecida ao executar o comando `net use` a partir de uma janela do PowerShell.
...
### Montar a partilha de ficheiros utilizando as credenciais persistentes
Assim que tiver uma ligação remota para o computador virtual, pode executar o comando `net use` para montar a partilha de ficheiros, utilizando a seguinte sintaxe. Substitua `<storage-account-name>` pelo nome da sua conta do Storage e `<share-name>` pelo nome da partilha do File Storage.
...
Uma vez que manteve as credenciais da conta do Storage no passo anterior, não tem de os fornecer com o comando `net use`. Se ainda não manteve as suas credenciais, em seguida, inclua-as como um parâmetro passado ao comando `net use`, conforme mostrado no exemplo seguinte.
...
Agora, pode trabalhar com a partilha do File Storage a partir do computador virtual como o faria com qualquer outra unidade. Pode emitir comandos de ficheiros padrão a partir da linha de comandos ou visualizar a partilha montada e o respetivo conteúdo a partir do Explorador de Ficheiros. De igual modo, pode executar código no computador virtual que acede à partilha de ficheiros através das APIs E/S dos ficheiros do Windows padrão, tais como as fornecidas pelos [espaços de nomes System.IO](http://msdn.microsoft.com/library/gg145019.aspx) no .NET Framework.
...
## Desenvolver com o File Storage
Para escrever código que chama o File Storage, pode utilizar as bibliotecas de cliente do armazenamento para o .NET e o Java ou a API REST do Storage do Azure. O exemplo nesta secção demonstra como trabalhar com uma partilha de ficheiros através da [Biblioteca de Clientes do Storage do Azure para o .NET](https://msdn.microsoft.com/library/mt347887.aspx) a partir de uma aplicação de consola simples em execução no ambiente de trabalho.
...
### Guardar as suas credenciais da conta do Storage no ficheiro app.config
Em seguida, guarde as suas credenciais no ficheiro app.config do seu projeto. Edite o ficheiro app.config para que seja apresentado de forma semelhante ao seguinte exemplo, substituindo `myaccount` pelo seu nome da conta do Storage e `mykey` pela sua chave da conta do Storage.
...
> [!NOTE]
> A versão mais recente do emulador do Storage do Azure não suporta o File Storage. A sua cadeia de ligação tem de visar uma conta do Storage do Azure na nuvem para funcionar com o File Storage.
> 
> 

### Adicionar declarações de espaço de nomes
Abra o ficheiro `program.cs` a partir do Explorador de Soluções e adicione as seguintes declarações de espaço de nomes para a parte superior do ficheiro.
...
Em seguida, adicione o seguinte código ao método `Main()` (depois do código mostrado acima) para obter a cadeia de ligação. Este código obtém uma referência para o ficheiro que criamos anteriormente e produz o respetivo conteúdo para a janela de consola.
...
Pode também utilizar o AzCopy para copiar um ficheiro para outro, copiar um blob para um ficheiro ou vice-versa. Consulte o artigo [Transferir dados com o Utilitário de Linha de Comandos AzCopy](storage-use-azcopy.md).

> [!NOTE]
> Se estiver a copiar um blob para um ficheiro ou um ficheiro para um blob, tem de utilizar uma assinatura de acesso partilhado (SAS) para autenticar o objeto de origem, mesmo se estiver a copiar dentro da mesma conta do Storage.
> 
> 

**Copiar um ficheiro para outro ficheiro**

O exemplo seguinte copia um ficheiro para outro ficheiro na mesma partilha. Uma vez que esta operação de cópia copia entre ficheiros na mesma conta do Storage, pode utilizar a autenticação de Chave Partilhada para efetuar a cópia.
...
O exemplo seguinte cria um ficheiro e copia-o para um blob na mesma conta do Storage. O exemplo cria um SAS para o ficheiro de origem, que o serviço utiliza para autenticar o acesso ao ficheiro de origem durante a operação de cópia.
...
## Resolução de problemas com o File Storage através de métricas
Agora, a Análise do Storage do Azure suporta métricas para o File Storage. Com os dados de métricas, pode rastrear pedidos e diagnosticar problemas.

Pode ativar métricas para o File Storage a partir do [Portal do Azure](https://portal.azure.com). Pode também ativar as métricas programaticamente chamando a operação Definir Propriedades do Serviço do Ficheiro através da API REST ou de um dos respetivos análogos na Biblioteca de Clientes do Storage.

O exemplo de código seguinte mostra como utilizar a Biblioteca de Clientes do Storage para o .NET para ativar as métricas para o File Storage.
...
9. **Uma operação de Mudança de nome também será adicionada à API REST?**
...
12. **O meu desempenho foi lento ao tentar deszipar ficheiros para o File Storage. O que devo fazer?**
    
    Para transferir um grande número de ficheiros para o File Storage, recomendamos que utilize o AzCopy, o Azure Powershell (Windows) ou a CLI do Azure (Linux/Unix), uma vez que estas ferramentas foram otimizadas para transferência de rede.
...
## Passos seguintes
Consulte as ligações para obter mais informações sobre o File Storage do Azure.

### Artigos e vídeos concetuais
* [File Storage do Azure: um sistema de ficheiros SMB em nuvem sem incómodos para o Windows e Linux](https://azure.microsoft.com/documentation/videos/azurecon-2015-azure-files-storage-a-frictionless-cloud-smb-file-system-for-windows-and-linux/)
* [Como utilizar o File Storage do Azure com o Linux](storage-how-to-use-files-linux.md)

### Suporte de ferramentas para o File Storage
* [Utilizar o Azure PowerShell com o Storage do Azure](storage-powershell-guide-full.md)
* [Como utilizar o AzCopy com o Armazenamento do Microsoft Azure](storage-use-azcopy.md)
* [Utilizar a CLI do Azure com o Storage do Azure](storage-azure-cli.md#create-and-manage-file-shares)

### Referência
* [Referência da Biblioteca de Clientes do Storage para o .NET](https://msdn.microsoft.com/library/azure/dn261237.aspx)
* [Referência da API REST do Serviço do Ficheiro](http://msdn.microsoft.com/library/azure/dn167006.aspx)
