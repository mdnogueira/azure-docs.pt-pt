<properties 
    pageTitle="Como criar uma aplicação Web com a Cache de Redis | Microsoft Azure" 
...
2. No **Explorador de Soluções**, faça duplo clique em **web.config** para abri-lo.

    ![Web.config][cache-web-config]

3.  Adicione a seguinte cadeia de ligação à secção `connectionStrings`. O nome da cadeia de ligação tem de corresponder ao nome da classe de contexto da base de dados do Entity Framework, que é `TeamContext`.
...
4. No **Explorador de Soluções**, expanda **Global.asax** e faça duplo clique em **Global.asax.cs** para abri-lo.

    ![Global.asax.cs][cache-global-asax]

5. Adicione as duas instruções de utilização que se seguem na parte superior do ficheiro, sob as outras declarações de utilização.
...
6. Adicione a seguinte linha de código no final do método `Application_Start`.


        Database.SetInitializer<TeamContext>(new TeamInitializer());


7. No **Explorador de Soluções**, expanda `App_Start` e faça duplo clique em `RouteConfig.cs`.

    ![RouteConfig.cs][cache-RouteConfig-cs]

8. Substitua `controller = "Home"` no seguinte código no método `RegisterRoutes` por `controller = "Teams"`, conforme mostrado no seguinte exemplo.
...
3. Na secção `body`, atualize a primeira instrução `Html.ActionLink` e substitua `Application name` por `Contoso Team Stats` e `Home` por `Teams`.
...
1. No computador, crie um ficheiro denominado `WebAppPlusCacheAppSecrets.config` e coloque-o numa localização que não será registada com o código fonte da aplicação de exemplo, caso decida registá-lo algures. Neste exemplo, o ficheiro `AppSettingsSecrets.config` está localizado em `C:\AppSecrets\WebAppPlusCacheAppSecrets.config`.
...
3. Adicione o seguinte atributo `file` ao elemento `appSettings`. Se utilizou um nome de ficheiro ou localização diferentes, substitua estes valores pelos valores mostrados no exemplo.
    -   Antes: `<appSettings>`
...
>[AZURE.NOTE] Não é necessário armazenar as estatísticas da equipa em vários formatos na cache para poder utilizar a Cache de Redis do Azure. Este tutorial utiliza vários formatos para demonstrar as diferentes formas e os diferentes tipos de dados que pode utilizar para colocar dados em cache.
...
3. Desloque-se para o fim do ficheiro **Index.cshtml** e adicione o seguinte elemento `tr`, de forma a ser a última linha na última tabela do ficheiro.
...
>[AZURE.NOTE] Se não tiver uma conta do Azure, pode [criar uma conta gratuita do Azure](https://azure.microsoft.com/pricing/free-trial/?WT.mc_id=redis_cache_hero) em apenas alguns minutos.
...
>[AZURE.IMPORTANT] Tenha em atenção que a eliminação de um grupo de recursos é irreversível e que o grupo de recursos e todos os recursos nele contidos serão permanentemente eliminados. Confirme que não elimina acidentalmente o grupo de recursos ou recursos errados. Se tiver criado os recursos para alojar este exemplo num grupo de recursos existente, pode eliminar cada recurso individualmente nos respetivos painéis.
...
>[AZURE.NOTE] Tenha em atenção que, uma vez que a aplicação, incluindo a base de dados, é executada localmente e a Cache de Redis está alojada no Azure, a cache poderá apresentar um desempenho inferior da base de dados. Para um melhor desempenho, a aplicação cliente e a instância da Cache de Redis do Azure devem estar na mesma localização. 
