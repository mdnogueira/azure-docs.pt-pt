...
Este tutorial demonstra como criar uma conta de base de dados do Azure Cosmos no portal do Azure e, em seguida, crie um Cassandra Table(documentdb-partition-data.md#partition-keys) utilizando o [Cassandra API](cassandra-introduction.md). Quando cria uma tabela se definir uma chave primária, a aplicação está preparada para dimensionamento effortlessly à medida que aumenta os dados. 
...
Agora pode importar dados adicionais para a coleção de BD do Cosmos do Azure. 
