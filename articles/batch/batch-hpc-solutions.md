<properties
   ...
   description="Apresenta cenários em lote e computação de alto desempenho (HPC e Macrocomputação) e opções de solução no Azure"
...
• **Macrocomputação** tende a envolver aplicações que dependem de energia e memória da CPU, tais como simulações de engenharia, modelação de riscos financeiros e composição digital. Os clusters que utilizam uma solução de Macrocomputação poderão incluir computadores com processadores especializados com vários núcleos para efetuar computação não processada, e hardware de rede de alta velocidade para ligar os computadores.
...
* [Rede Virtual](https://azure.microsoft.com/documentation/services/virtual-network/) - cria uma secção logicamente isolada no Azure para ligar recursos do Azure ao seu centro de dados no local ou um único computador cliente com o IPSec; permite que as aplicações de Macrocomputação acedam a dados no local, serviços do Active Directory e servidores de licenças
...
* [Barramento de Serviço](https://azure.microsoft.com/documentation/services/service-bus/) - fornece vários mecanismos para que as aplicações comuniquem ou troquem dados, quer estes estejam no Azure, noutra plataforma em nuvem ou num centro de dados.
