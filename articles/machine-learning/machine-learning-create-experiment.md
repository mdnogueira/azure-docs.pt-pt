...
> [AZURE.TIP] Apagar os valores em falta a partir dos dados de entrada é um pré-requisito para utilizar a maioria dos módulos.
...
> [AZURE.TIP] Utilize o botão **GUARDAR COMO** na tela de experimentação para copiar qualquer iteração da sua experimentação. Pode ver todas as iterações da sua experimentação, clicando em **VER HISTÓRICO DE EXECUÇÕES** por baixo da tela. Para obter mais informações, consulte [Gerir iterações de experimentação no Azure Machine Learning Studio][históricoexecuções].
