...
Estas instruções começa por que descrevem os pré-requisitos e os recursos que são necessárias para concluir as tarefas de Data Lake Analytics que formam o processo de ciência de dados e como instalá-los. -Descreve os passos de processamento de dados, utilizando U-SQL e é concluída ao que mostra como utilizar o Python e o Hive com o Azure Machine Learning Studio para criar e implementar os modelos preditivos. 
...
> [!NOTE]
> Em vez do Visual Studio, também pode utilizar o Portal do Azure para submeter consultas do Azure Data Lake. Será, fornecemos instruções sobre como efetuar, por isso, ambos com o Visual Studio e no portal na secção intitulada **processar os dados com o U-SQL**. 
> 
> 
...
Esta secção fornece instruções sobre como criar cada um destes recursos. Se optar por utilizar as tabelas do Hive com o Azure Machine Learning, em vez de Python, para criar um modelo, também terá de aprovisionar um cluster de HDInsight (Hadoop). Este procedimento alternativo descrito na secção adequada abaixo.


> [!NOTE]
> O **Azure Data Lake Store** pode ser criado em separado ou ao criar o **Azure Data Lake Analytics** como armazenamento de predefinido. São referenciadas instruções para criar cada um destes recursos separadamente abaixo, mas a conta de armazenamento do Data Lake não precisa de ser criada em separado.
>
> 
...
Da mesma forma, pode ler os conjuntos de dados fare. Clique com o botão direito do rato em Azure Data Lake Store, pode optar por observar os dados no **Portal do Azure--> Explorador de dados** ou **Explorador de ficheiros** no Visual Studio. 
...
Podemos fazer algumas exploração de dados para obter uma melhor compreensão dos dados.
...
Localizar a distribuição da quantidade de sugestão com valores de truncado: 0,5,10 e utilizados no compromisso 20.
...
Primeiro vamos aleatoriamente selecione 0.1% dos dados da tabela associada:
...
Quando a tarefa é complied com êxito, o estado da tarefa será apresentado no Visual Studio para monitorização. Após a tarefa em execução, pode ainda o processo de execução da tarefa de repetição e saber os passos de congestionamento para melhorar a eficiência da tarefa. Também pode aceder ao Portal do Azure para verificar o estado das suas tarefas U-SQL.

 ![13](./media/data-lake-walkthrough/13-USQL-running-v2.PNG)

 ![14](./media/data-lake-walkthrough/14-USQL-jobs-portal.PNG)

Agora pode verificar os ficheiros de saída no Blob storage do Azure ou no Portal do Azure. Utilizaremos os dados de exemplo stratified para a nossa modelação no próximo passo.
...
Iremos demonstrar duas opções disponíveis para si extrair dados para o Azure Machine Learning para criar e 
...
Aqui vamos construir um modelo de classificação binária para prever se um viagem é tipped ou não. O bloco de notas do Jupyter pode encontrar outros dois modelos: classificação várias classes e modelos de regressão.

* Primeiro é preciso criar variáveis fictícias que podem ser utilizadas no scikit-saiba modelos
...
Queremos operacionalizar o modelo de machine learning depois de ter sido concebido. Aqui, utilizamos o modelo de logística da binário como exemplo. Certifique-se de que o scikit-saber versão no seu computador local é 0.15.1. Não tem de preocupar com esta se utilizar o serviço do Azure ML studio.
...
Agora vamos criar as tabelas do Hive a ser utilizado no Azure Machine Learning Studio no cluster do HDInsight com os dados armazenados no Azure Data Lake Store no passo anterior. Aceda ao cluster do HDInsight que acabou de criar. Clique em **definições** --> **propriedades** --> **identidade do AAD Cluster** --> **acesso ADLS**, certifique-se a sua conta do Azure Data Lake Store é adicionada na lista com leitura, escrita e direitos de execução. 

 ![19](./media/data-lake-walkthrough/19-HDI-cluster-add-ADLS.PNG)

Em seguida, clique em **Dashboard** junto a **definições** botão e uma janela irão aparecer. Clique em **vista do Hive** no canto superior direito da página e o utilizador irá ver o **Editor de consultas**.
...
Quando a consulta estiver concluída em execução, verá os resultados da seguinte forma:
...
Iremos agora está prontos para criar e implementar um modelo que prevê ou não uma sugestão é paga com o Azure Machine Learning. Os dados de exemplo stratified estão prontos para ser utilizado nesta classificação binária (sugestão ou não) problema. Modelos preditivos através de várias classes classificação (tip_class) e regressão (tip_amount) também podem ser criados e implementados com o Azure Machine Learning Studio, mas aqui vamos mostrar apenas como lidar com as maiúsculas e minúsculas utilizando o modelo de classificação binária.
...
Um exemplo de uma experimentação de classificação binária ler os dados da tabela do Hive é mostrado na imagem abaixo.
...
Dashboard do serviço web será apresentado em breve:
