...
Pode criar aplicações do Service Fabric para serem executadas em clusters do Linux através do Mac OS X. Este documento descreve como configurar o seu Mac para desenvolvimento.

## <a name="prerequisites"></a>Pré-requisitos
O Service Fabric não é executado de forma nativa no OS X. Para executar um cluster do Service Fabric local, é fornecida uma imagem de contentor do Docker pré-configurada. Antes de começar, vai precisar:
...
2. Atualize a configuração do daemon do Docker no seu anfitrião com o seguinte e reinicie-o: 
    ...
    ```
    Pode atualizá-lo diretamente no daemon.json no caminho da instalação do docker (a localização pode variar de computador para computador, como por exemplo, ~/Library/Containers/com.docker.docker/Data/database/com.docker.driver.amd64-linux/etc/docker/daemon.json). A forma recomendada de atualizar é ir a Ícone do Docker > Preferências > Daemon > Avançadas e atualizar aí.
...
6. Quando o passo 5 estiver devidamente concluído, pode ir a ``http://localhost:19080`` a partir do seu Mac, onde pode ver o explorador do Service Fabric.
...
1. Tem de ter o Node.js e o NPM instalados no Mac. Caso contrário, pode utilizar o passo seguinte para instalá-los com o Homebrew:
