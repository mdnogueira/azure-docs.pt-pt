...
## Descrição geral do Centro de Segurança do Azure
...
## Guia de planeamento
Este guia aborda um conjunto de passos e tarefas que pode seguir para otimizar a utilização do Centro de Segurança do Azure com base nos requisitos de segurança da sua organização e o modelo de gestão de nuvem. Para tirar o máximo partido do Centro de Segurança do Azure, é importante compreender de que forma diferentes pessoas ou equipas na sua organização vão utilizar o serviço para dar resposta às necessidades de desenvolvimento e operações seguras, de monitorização, administração e de resposta a incidentes. As principais áreas a considerar quando planear utilizar o Centro de Segurança do Azure são:
...
O Centro de Segurança do Azure permite que estes indivíduos cumpram estas variadas responsabilidades. Por exemplo:
...
- Também Pode Utilizar um Sistema de Emissão de Permissões para Registar Alterações (preenche recomendações através da API)
...
- Pode Utilizar um Dashboard Existente (preenche alertas através da API)
...
O Centro de Segurança do Azure utiliza o [Controlo de Acesso Baseado em Funções (RBAC)](../active-directory/role-based-access-control-configure.md), que fornece [funções incorporadas](../active-directory/role-based-access-built-in-roles.md) que podem ser atribuídas a utilizadores, grupos e serviços no Azure. Quando um utilizador abre o Centro de Segurança do Azure, apenas verá informações relacionadas com os recursos aos quais tenha acesso, o que significa que o utilizador tem atribuída a função de Proprietário, Contribuinte ou Leitor para a subscrição ou o grupo de recursos a que um recurso pertence. Ao utilizar as pessoas fictícias acima, seria necessário o seguinte RBAC:
...
Quando planear o controlo de acesso ao utilizar o RBAC para o Centro de Segurança do Azure, certifique-se de que compreende quem é que na sua organização vai utilizar o Centro de Segurança do Azure e quais são os tipos de tarefas que vão realizar. Em seguida, configure o RBAC em conformidade. 
...
## Recomendações e políticas de segurança
Uma política de segurança define o conjunto de controlos que são recomendados para recursos dentro da subscrição especificada ou do grupo de recursos. No Centro de Segurança do Azure, é possível definir políticas de acordo com os requisitos de segurança da sua empresa e com o tipo de aplicações ou sensibilidade dos dados. 
...
**Endpoint Protection**: se uma máquina virtual não tiver uma solução de proteção de ponto final ativada, o Centro de Segurança do Azure recomendará que instale uma. Se tiver uma solução de proteção de ponto final preferencial que já tenha adotado no local, terá de decidir se irá utilizar o mesmo antimalware para as suas VMs do Azure. O Centro de Segurança do Azure fornece-lhe várias opções de proteção de ponto final.  Pode utilizar o Microsoft Antimalware gratuito ou escolher entre uma lista de soluções de proteção de ponto final de parceiros integrados. Para obter mais informações sobre como implementar o antimalware através do Centro de Segurança do Azure, leia [Ativar antimalware no Centro de Segurança do Azure](security-center-enable-antimalware.md).

**Atualizações do Sistema**: o Centro de Segurança do Azure identificará as máquinas virtuais nas quais estejam em falta atualizações do sistema operativo críticas ou de segurança. Considere quem será o responsável pela aplicação de atualizações quando necessário e como serão aplicadas. Muitas organizações utilizam os WSUS, o Windows Update ou outra ferramenta.
...
Se estiver a utilizar uma conta do Storage partilhada entre diferentes recursos do Azure, certifique-se de que lê o artigo [Metas de Desempenho e Escalabilidade do Storage do Azure](../storage/storage-scalability-targets.md) para obter mais informações sobre limites e restrições de tamanho. A sua subscrição também tem limites de contas do Storage. Reveja [Limites, quotas e restrições de subscrição e serviço do Azure](../azure-subscription-service-limits) para compreender melhor estes limites.
...
Após a configuração inicial e a aplicação das recomendações do Centro de Segurança do Azure, o passo seguinte é considerar os processos operacionais do Centro de Segurança do Azure. 

Para aceder ao Centro de Segurança do Azure a partir do Portal do Azure, pode clicar em **Procurar** e escrever **Centro de Segurança** no campo do **Filtro**. As vistas que o utilizador obtém estão de acordo com estes filtros aplicados.
...
Quando adicionar novos recursos (VMs, BDs SQL) ao seu Ambiente do Azure, o Centro de Segurança do Azure irá detetar automaticamente estes recursos e começar a monitorizar a segurança dos mesmos. Se a Recolha de Dados estiver ativada na Política de Segurança, serão ativadas automaticamente capacidades de monitorização adicionais para as suas máquinas virtuais. 

![Áreas principais](./media/security-center-planning-and-operations-guide/security-center-planning-and-operations-guide-fig3.png) 
...
4.  À medida que são adicionados novos recursos ao seu ambiente do Azure, é possível que apareçam novos alertas no mosaico **Alertas de Segurança**. Verifique sempre se existem novos alertas neste mosaico e tome medidas de acordo com as recomendações do Centro de Segurança do Azure.

Também é aconselhável que monitorize regularmente o estado dos recursos existentes para identificar as alterações de configuração que criaram riscos de segurança, que se desviam das linhas de base recomendadas e dos alertas de segurança. Comece no dashboard do Centro de Segurança do Azure. A partir daí tem três áreas principais para rever de forma consistente.
...
2.  O painel **Recomendações** permite-lhe rever as recomendações do Centro de Segurança do Azure. Durante a monitorização em curso pode considerar que não tem recomendações diariamente, o que é normal, uma vez que abordou todas as recomendações na configuração inicial do Centro de Segurança do Azure. Por este motivo, é possível que não tenha novas informações nesta secção todos os dias e que apenas tenha de aceder à mesma conforme seja necessário.
3.  O painel **Deteção** pode ser alterado de forma bastante frequente ou pouco frequente. Reveja sempre os alertas de segurança e tome medidas com base nas recomendações do Centro de Segurança do Azure. 

## Resposta a Incidentes

O Centro de Segurança do Azure deteta e alerta-o relativamente a ameaças à medida que ocorrem. As organizações devem monitorizar a existência de novos alertas de segurança e tomar medidas conforme seja necessário para uma investigação mais aprofundada ou para remediar o ataque.

> [AZURE.NOTE] Este artigo não tem a intenção de o ajudar a criar o seu plano de Resposta a Incidentes. Pode utilizar o [Computer Security Incident Handling Guide](http://nvlpubs.nist.gov/nistpubs/SpecialPublications/NIST.SP.800-61r2.pdf) (Guia de Processamento de Incidentes de Segurança Informática) do National Institute of Standards and Technology (NIST) (Instituto Nacional de Normas e Tecnologia dos Estados Unidos) como referência para o ajudar a criar o seu próprio.

Cada Alerta de Segurança faculta informações que podem ser utilizadas para compreender melhor a natureza do ataque e sugerir possíveis mitigações. Alguns alertas também fornecem ligações para mais informações ou para outras fontes de informação no Azure. Pode utilizar as informações fornecidas para uma investigação mais ponderada e para iniciar a mitigação.
...
| **Nível**        | **Deteção de Ataques**                     | **Mitigação Proposta**                                                                       |
|------------------|------------------------------------------|-----------------------------------------------------------------------------------------------|
| Rede          | Deteção de força bruta de RDP                | Reduza a superfície de ataque ao remover pontos finais abertos desnecessários, configure ACLs, utilize uma palavra-passe segura |
| Virtual Machines | SVCHOST em execução a partir do diretório errado | Mova a VM para uma sub-rede diferente (isolada), analise-a e recrie-a,                        |
| Aplicação      | Injeção de SQL na Base de Dados do Azure          | Proteger configurações de bases de dados                                                                |
