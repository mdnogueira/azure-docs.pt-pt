<properties 
    pageTitle="Tutorial de ASP.NET MVC para DocumentDB: Programação de Aplicações Web | Microsoft Azure" 
    description="Tutorial de ASP.NET MVC para criar uma aplicação Web de MVC ao utilizar DocumentDB. Irá armazenar dados de acesso e JSON a partir de uma aplicação de tarefas alojada no em Web Sites do Azure – tutorial ASP NET MVC passo a passo." 
...
Esta orientação mostra-lhe como utilizar o serviço do DocumentDB fornecido pelo para armazenar e aceder a dados a partir de uma aplicação Web ASP.NET MVC alojada no Azure. Se estiver à procura de um tutorial que se concentra apenas no DocumentDB e não nos componentes do ASP.NET MVC, consulte [Criar uma aplicação de consola C# do DocumentDB](documentdb-get-started.md).

> [AZURE.TIP] Este tutorial parte do pressuposto de que tem alguma experiência anterior na utilização do ASP.NET MVC e de Web Sites do Azure. Se não estiver familiarizado com o ASP.NET ou com as [ferramentas dos pré-requisitos](#_Toc395637760), recomendamos que transfira todo o projeto de exemplo a partir do [GitHub][] e que siga as instruções neste exemplo. Assim que o tiver criado, pode rever este artigo para obter conhecimentos aprofundados sobre o código no contexto do projeto.
...
Comecemos por criar uma conta DocumentDB. Se já tiver uma conta, pode avançar para o [Criar uma nova aplicação ASP.NET MVC](#_Toc395637762).
...
6. Se pretende alojar a sua aplicação no Azure, selecione **Anfitrião na nuvem** no canto inferior direito para que o Azure aloje a aplicação. Selecionamos o alojamento na nuvem e a execução da aplicação alojada num Web Site do Azure. A seleção desta opção irá aprovisionar antecipadamente um Web Site do Azure por si e facilitar-lhe muito mais a vida quando chegar o momento de implementar a aplicação de trabalho definitiva. Se pretende alojá-lo noutro local ou não pretende configurar o Azure antecipadamente, basta desmarcar **Anfitrião na Nuvem**.

7. Clique em **OK** e permita que o Visual Studio atue em redor do andaime no modelo em branco do ASP.NET MVC. 

8. Se tiver optado por alojá-lo na nuvem, verá pelo menos um ecrã adicional a pedir-lhe para iniciar sessão na sua conta do Azure e facultar alguns valores para o seu novo site. Faculte todos os valores adicionais e continue. 

    Ainda não escolhi um “Servidor de base de dados” aqui porque não estamos a utilizar um Servidor de Base de Dados SQL do Azure. Vamos criar uma nova conta do Azure DocumentDB mais tarde no Portal do Azure.

    Para obter mais informações sobre como escolher um **plano do Serviço de Aplicações** e um **Grupo de recursos**, consulte [Descrição geral aprofundada dos planos do Serviço de Aplicações do Azure](../app-service/azure-web-sites-web-hosting-plans-in-depth-overview.md).

    ![Captura de ecrã da caixa de diálogo Configurar Site do Microsoft Azure](./media/documentdb-dotnet-application/image11_1.png)

9. Assim que o Visual Studio tenha terminado de criar a aplicação MVC automático, fica com uma aplicação ASP.NET vazia que pode executar localmente.
...
2. Na caixa **Procurar** do NuGet, escreva ***Azure DocumentDB***.
    ...
    Nos resultados, instale o pacote **Biblioteca de Clientes do Microsoft Azure DocumentDB**. Isto irá transferir e instalar o pacote do DocumentDB, bem como todas as dependências, como Newtonsoft.Json. Clique em **OK** na janela **Pré-visualizar** e em **Aceito** na janela **Aceitação de Licença** para concluir a instalação.
...
1. No **Explorador de Soluções**, clique com o botão direito do rato na pasta **Controladores**, clique em **Adicionar** e, em seguida, em **Controlador**.
    ...
    A caixa de diálogo **Adicionar Andaime** é apresentada.

2. Selecione **Controlador MVC 5 – Vazio** e, em seguida, clique em **Adicionar**.

    ![Captura de ecrã da caixa de diálogo Adicionar Andaime com a opção Controlador MVC 5 – Vazio realçada](./media/documentdb-dotnet-application/image14.png)
...
3. Atribua o nome **ItemController** ao novo Controlador.
    ...
    Depois de ficheiro estar criado, a sua solução Visual Studio deve assemelhar-se ao seguinte com o novo ficheiro ItemController.cs no **Explorador de Soluções**. Também é apresentado o novo ficheiro Item.cs criado anteriormente.
...
2. Na caixa de diálogo **Adicionar Vista**, efetue o seguinte:
...
Agora que o conteúdo MVC standard está tratado, vamos adicionar o código para o DocumentDB. 
...
O primeiro a fazer é adicionar uma classe que contém toda a lógica para estabelecer a ligação ao DocumentDB e utilizá-lo. Neste tutorial vamos encapsulam toda esta lógica para uma classe de repositório denominada DocumentDBRepository. 
...
2. Na classe recém-criada **DocumentDBRepository** adicione as seguintes *instruções using* acima da declaração *espaço de nomes*
    ...
    > [AZURE.TIP] Ao criar um novo DocumentCollection, pode fornecer um parâmetro RequestOptions opcional de OfferType, que lhe permite especificar o nível de desempenho da nova coleção. Se este parâmetro não for passado, será utilizado o tipo de oferta predefinido. Para obter mais informações sobre os tipos de oferta do DocumentDB, consulte [Níveis de Desempenho do DocumentDB](documentdb-performance-levels.md)
...
4. Agora, atualize os valores para *ponto final* e *authKey* ao utilizar o painel Chaves do Portal do Azure. Utilize o **URI** a partir do painel Chaves como o valor da definição do ponto final e utilize a **CHAVE PRIMÁRIA** ou a **CHAVE SECUNDÁRIA** a partir do painel Chaves como o valor da definição authKey.


    A preparação do repositório do DocumentDB fica assim resolvida. Agora vamos adicionar a nossa lógica da aplicação.

5. A primeira coisa que queremos conseguir fazer com uma aplicação de lista de tarefas é apresentar os itens incompletos.  Copie e cole o seguinte fragmento de código em qualquer lugar dentro da classe **DocumentDBRepository**.
...
6. Abra o **ItemController** que adicionamos anteriormente e adicione as seguintes *instruções using* acima da declaração de espaço de nomes.

        using System.Net;
        using System.Threading.Tasks;
        using todo.Models;

    Se o seu projeto não se chamar “todo” (tarefa), tem de atualizar utilizando “todo.Models” de modo a refletir o nome do seu projeto.
...
Agora é indicado ao ASP.NET MVC que, se não especificou um valor no URL para controlar o comportamento de encaminhamento, em vez de **Base**, utiliza-se **Item** como o controlador e **Índice** de utilizador como a vista.
...
2. Adicione o próximo bloco de código à classe ItemController.cs que indica ao ASP.NET MVC o que fazer com um POST de formulário para este controlador.
    ...
    **Nota de Segurança**: o atributo **ValidateAntiForgeryToken** é utilizado aqui para ajudar a proteger esta aplicação contra ataques de falsificação de pedidos entre sites. Não basta apenas adicionar este atributo, as suas vistas também têm de trabalhar com este token antifalsificação. Para obter mais informações sobre o assunto e exemplos sobre como implementar isto corretamente, consulte [Prevenir Falsificação de Pedidos Entre Sites][]. O código de origem fornecido no [GitHub][] tem a implementação completa no local.

    **Nota de Segurança**: também utilizamos o atributo **Vincular** no parâmetro do método para ajudar a proteger contra ataques de publicação excessiva. Para obter mais detalhes, consulte [Operações CRUD Básicas no ASP.NET MVC][].
...
2. Adicione o seguinte à classe **ItemController**.
    ...
    O primeiro método processa o Http GET que ocorre quando o utilizador clica na ligação **Editar** a partir da vista **Índice**. Este método obtém um [**Documento**](http://msdn.microsoft.com/library/azure/microsoft.azure.documents.document.aspx) do DocumentDB e passa-o para a vista **Editar**.

    A vista **Editar** efetua então um Http POST para o **IndexController**. 
    
    O segundo método que adicionamos processa a passagem do objeto atualizado para o DocumentDB de modo a manter-se na base de dados.
...
Para testar a aplicação no seu computador local, efetue o seguinte:
...
3. Clique em **Editar** junto a um **Item** na lista para ser direcionado para a vista **Editar**, onde pode atualizar qualquer propriedade do seu objeto, incluindo o sinalizador **Concluído**. Se marcar o sinalizador **Concluído** e clicar em **Guardar**, o **Item** é removido da lista de tarefas incompletas.
...
Para adicionar mais funcionalidades à sua aplicação, reveja as APIs disponíveis na [Biblioteca do DocumentDB .NET](https://msdn.microsoft.com/library/azure/dn948556.aspx) e esteja à vontade para contribuir para a Biblioteca do DocumentDB .NET no [GitHub][]. 
