...
Este tutorial mostra como utilizar as ferramentas do Visual Studio que ajudem a depurar uma aplicação web no [do serviço de aplicações](http://go.microsoft.com/fwlink/?LinkId=529714), executando na [modo de depuração](http://www.visualstudio.com/get-started/debug-your-app-vs.aspx) remotamente ou visualizando os registos de aplicações e os registos do servidor web.
...
O tutorial parte do princípio de que está a utilizar o Visual Studio 2015 ou 2013. Se estiver a utilizar o Visual Studio 2013, as funcionalidades de WebJobs requerem [Update 4](http://go.microsoft.com/fwlink/?LinkID=510314) ou posterior.
...
3. Expanda o grupo de recursos que inclui a aplicação web que criou no [introdução ao Azure e ao ASP.NET][GetStarted]e, em seguida, clique no nó da aplicação web e clique em **ver definições**.
    ...
    Para obter informações sobre as caixas de cadeias de ligação e as definições de aplicação nesta janela, consulte [Web Apps do Azure: como as cadeias de aplicação e de trabalho de cadeias de ligação](http://blogs.msdn.com/b/windowsazure/archive/2013/07/17/windows-azure-web-sites-how-application-strings-and-connection-strings-work.aspx).
...
Esta secção mostra como depurar remotamente utilizando o projeto que criar no [introdução ao Azure e ao ASP.NET][GetStarted].

1. Abra o projeto web que criou no [introdução ao Azure e ao ASP.NET][GetStarted].
...
6. No **perfil** na lista pendente, selecione o mesmo perfil que é utilizado em [introdução ao Azure e ao ASP.NET][GetStarted].
7. Clique em de **definições** separador e alterar **configuração** para **depurar**e, em seguida, clique em **publicar**.

    ![Publicar no modo de depuração](./media/web-sites-dotnet-troubleshoot-visual-studio/tws-publishdebug.png)
8. Após a implementação estiver concluído e o browser abre-se ao URL do Azure da sua aplicação web, feche o browser.
9. No **Explorador de servidores**, a aplicação web com o botão direito e, em seguida, clique em **anexar depurador**.
    ...
    O browser abre-se automaticamente à sua home page em execução no Azure. Poderá ter de aguardar 20 segundos ou, pelo que, enquanto o Azure configura o servidor de depuração. Este atraso só acontece da primeira vez que executa no modo de depuração numa aplicação web. Subsequentes vezes, num próximas 48 horas quando iniciar novamente depuração será um atraso.

    **Nota:** se tiver quaisquer problemas em iniciar o depurador, tentar fazê-lo utilizando **Cloud Explorer** em vez de **Explorador de servidores**.
...
8. Clique em **anexar depurador**.
    ...
    O browser abre-se automaticamente à sua home page em execução no Azure. Poderá ter de aguardar 20 segundos ou, pelo que, enquanto o Azure configura o servidor de depuração. Este atraso só acontece da primeira vez que executa no modo de depuração numa aplicação web. Da próxima vez que ligue o depurador não existe será um atraso se fazê-lo no prazo de 48 horas.
9. No browser que é aberto para a home page de anúncios da Contoso, crie um novo anúncio.

    Criar um anúncio faz com que uma mensagem de fila ser criado, que irá ser captada pelo trabalho Web e processada. Quando o SDK de WebJobs chama a função para processar a mensagem da fila, o código será atingiu o seu ponto de interrupção.
10. Quando interrompe o depurador, o ponto de interrupção, pode examinar e alterar os valores das variáveis enquanto o programa está a executar a nuvem. Na ilustração seguinte, o depurador mostra os conteúdos do objeto blobInfo que foi transmitido para o método GenerateThumbnail.
...
11. Prima F5 para continuar a ser executado.
     ...
     O método GenerateThumbnail acaba de criar a miniatura.
...
15. Inicie sessão com as suas credenciais do Azure e, em seguida, clique no nome do WebJob para ir para a página para o trabalho Web.
     ...
     O Dashboard mostra que a função de GenerateThumbnail executada recentemente.
...
* Não é recomendado em execução no modo de depuração na produção. Se a sua aplicação web de produção não é ampliar a várias instâncias de servidor, depuração irá impedir que o servidor web de responder a pedidos de outros. Se tiver várias instâncias do servidor web, quando ligar para o depurador irá obter uma instância aleatória e não tem nenhuma forma para se certificar de que o browser subsequentes pedidos passará a essa instância. Além disso, normalmente, não implementa uma compilação de depuração para produção e otimizações de compilador para compilações de versão poderão executá-lo Mostrar o que acontece linha por linha no seu código de origem. Para resolver problemas de produção, o seu recurso melhor é aplicação web e de rastreio de registos do servidor.
...
* **Registos de mensagem de erro detalhadas**<br/>
  O servidor web cria uma página HTML com algumas informações adicionais para pedidos HTTP falhados (aqueles que resultam num código de estado 400 ou superior).
...
5. Na barra de endereço da janela do browser, adicione *trace.axd* para o URL e, em seguida, prima Enter (o URL será semelhante ao http://localhost:53370/trace.axd).
6. No **aplicação rastreio** página, clique em **ver detalhes** na primeira linha (não a linha BrowserLink).
    ...
    No entanto, ativar `trace.axd` um produção aplicação web, geralmente, não é recomendada por motivos de segurança e as secções seguintes, verá uma forma mais fácil de ler os registos de rastreio numa aplicação web do Azure.
...
4. Na janela do browser mostra a home page da aplicação, clique em **contacte**.
    ...
    Dentro de alguns segundos a saída do nível de erro que adicionou ao analisar o `Contact` método aparece no **saída** janela.
...
2. No **saída** janela, clique em de **especificar que os registos do Azure para monitorizar** botão.

    ![Especificar os registos do Azure para monitorizar](./media/web-sites-dotnet-troubleshoot-visual-studio/tws-specifylogs.png)
3. No **as opções de registo do Azure** caixa de diálogo, selecione **registos do servidor de Web**e, em seguida, clique em **OK**.
...
2. No **saída** janela, clique em de **especificar que os registos do Azure para monitorizar** botão.
3. No **as opções de registo do Azure** caixa de diálogo, clique em **todos os registos**e, em seguida, clique em **OK**.
...
4. Na barra de endereço da janela do browser, adicione um caráter adicional para o URL de causar um erro 404 (por exemplo, `http://localhost:53370/Home/Contactx`), e prima Enter.
    ...
    Depois de vários segundos o registo de erros detalhados aparece no Visual Studio **saída** janela.

    ![Registo de erros detalhados na janela de saída](./media/web-sites-dotnet-troubleshoot-visual-studio/tws-detailederrorlog.png)

    Controlar + clique na ligação para ver a saída de registo a formatados num browser:

    ![Registo de erros detalhados na janela do browser](./media/web-sites-dotnet-troubleshoot-visual-studio/tws-detailederrorloginbrowser.png)
...
Os registos de rastreio de aplicações também podem ser enviados para uma conta de armazenamento do Azure e pode visualizá-los no Visual Studio. Para que irá criar uma conta de armazenamento, ativar registos de armazenamento no portal clássico e visualizá-las no **registos** separador do **aplicação Web do Azure** janela.

Pode enviar os registos a qualquer um ou todos os destinos de três:

* O sistema de ficheiros.
* Tabelas de conta de armazenamento.
* Blobs de conta de armazenamento.

Pode especificar um nível de gravidade de diferente para cada destino.

As tabelas tornam mais fácil ver os detalhes dos registos online e suporta a transmissão em fluxo; Pode consultar os registos em tabelas e ver novos registos, que estão a ser criadas. Os BLOBs tornam mais fácil para transferir os registos em ficheiros e analisá-las com o HDInsight, porque o HDInsight sabe como trabalhar com o blob storage. Para obter mais informações, consulte **Hadoop e MapReduce** no [opções de armazenamento de dados (compilar aplicações de nuvem de mundo Real com o Azure)](http://www.asp.net/aspnet/overview/developing-apps-with-windows-azure/building-real-world-cloud-apps-with-windows-azure/data-storage-options).

Atualmente, possui os registos do sistema de ficheiros, definidos ao nível verboso; os seguintes passos guiá-lo a configurar os registos de nível de informações para aceder a tabelas de conta de armazenamento. Nível de informações significa todos os registos criados chamando `Trace.TraceInformation`, `Trace.TraceWarning`, e `Trace.TraceError` será apresentado, mas não os registos criados chamando `Trace.WriteLine`.

As contas de armazenamento oferecem mais armazenamento e de retenção mais longo lasting para os registos em comparação comparada o sistema de ficheiros. Outra vantagem de envio de registos de rastreio de aplicação para o armazenamento é que obter algumas informações adicionais com cada registo que não aproveitar os registos do sistema de ficheiros.

1. Clique com botão direito **armazenamento** sob o nó do Azure e, em seguida, clique **criar conta de armazenamento**.

![Criar conta de armazenamento](./media/web-sites-dotnet-troubleshoot-visual-studio/createstor.png)

1. No **criar conta de armazenamento** caixa de diálogo, introduza um nome para a conta de armazenamento.

    Tem de ser o nome tem de ser exclusivo (nenhuma conta de armazenamento do Azure pode ter o mesmo nome). Se o nome introduzido já se encontra em utilização, obterá uma oportunidade para alterá-la.

    O URL para aceder à sua conta do storage será *{name}*. core.windows.net.
2. Definir o **região ou grupo de afinidade** na lista pendente para a região mais próxima de si.

    Esta definição especifica em que datacenter do Azure irá alojar a sua conta do storage. Para este tutorial à sua escolha não tornar uma diferença percetível, mas para uma aplicação web de produção pretende que o servidor web e a sua conta de armazenamento deve estar na mesma região para minimizar a latência e dados de encargos associados à saída. A aplicação web (o que criará mais tarde) deve ser executada numa região como fechar quanto possível para browsers aceder à sua aplicação web para minimizar a latência.
3. Defina a lista pendente **Replicação** para **Localmente redundante**.
...
4. Clique em **Criar**.
    ...
    ![Nova conta do Storage](./media/web-sites-dotnet-troubleshoot-visual-studio/newstorage.png)    
5. No Visual Studio **aplicação Web do Azure** janela, clique em de **registos** separador e, em seguida, clique em **configurar registo no Portal de gestão**.

    <!-- todo:screenshot of new portal if the VS page link goes to new portal -->
    ![Configurar o registo](./media/web-sites-dotnet-troubleshoot-visual-studio/tws-configlogging.png)

    Esta ação abre o **configurar** separador no portal clássico para a sua aplicação web.
6. No portal clássico **configurar** separador, desloque para baixo para a secção de diagnóstico de aplicação e, em seguida, altere **registo na aplicação (Table Storage)** para **no**.
7. Alteração **nível de registo** para **informações**.
8. Clique em **gerir o armazenamento de tabelas**.

    ![Clique em Gerir TableStorage](./media/web-sites-dotnet-troubleshoot-visual-studio/tws-stgsettingsmgmtportal.png)

    No **gerir o armazenamento de tabela para o application diagnostics** caixa, pode escolher a sua conta de armazenamento se tiver mais do que um. Pode criar uma nova tabela ou utilize uma já existente.

    ![Gerir o table storage](./media/web-sites-dotnet-troubleshoot-visual-studio/tws-choosestorageacct.png)
9. No **gerir o armazenamento de tabela para o application diagnostics** caixa clique na marca de verificação para fechar a caixa.
10. No portal clássico **configurar** separador, clique em **guardar**.
11. Na janela do browser que apresenta a aplicação web de aplicação, clique em **home page**, em seguida, clique em **sobre**e, em seguida, clique em **contacte**.

     As informações de registo produzidas navegando estas páginas web serão escritas para a conta de armazenamento.
12. No **registos** separador do **aplicação Web do Azure** janela no Visual Studio, clique em **atualizar** em **resumo diagnóstico**.

     ![Clique em Atualizar](./media/web-sites-dotnet-troubleshoot-visual-studio/tws-refreshstorage.png)

     O **resumo diagnóstico** secção mostra os registos para os últimos 15 minutos, por predefinição. Pode alterar o período de para ver mais registos.

     (Se obtiver um erro "não foi localizada a tabela", certifique-se de que navegado para as páginas que efetue o rastreio depois de ativado **(armazenamento) de registo de aplicação** e depois em que clicou **guardar**.)

     ![Registos de armazenamento](./media/web-sites-dotnet-troubleshoot-visual-studio/tws-storagelogs.png)

     Tenha em atenção que neste ver consulte **ID do processo** e **ID do Thread** para cada registo, que não a obter nos registos de sistema de ficheiros. Pode ver os campos adicionais, diretamente a visualização da tabela de armazenamento do Azure.
13. Clique em **ver todos os registos de aplicação**.

     A tabela de registo de rastreio é apresentado no Visualizador de tabela de armazenamento do Azure.

     (Se obtiver um erro de "sequência não contém elementos", abra **Explorador de servidores**, expanda o nó para a sua conta de armazenamento sob o **Azure** nó e, em seguida, contexto **tabelas**e clique em **atualizar**.)

     ![Os registos de armazenamento na vista em tabela](./media/web-sites-dotnet-troubleshoot-visual-studio/tws-tracelogtableview.png)

     Esta vista mostra os campos adicionais que não for apresentada em todas as outras vistas. Esta vista também lhe permite filtrar registos utilizando a IU de construtor de consultas especiais para construir uma consulta. Para obter mais informações, consulte a trabalhar com recursos de Table - filtragem entidades no [navegação nos recursos de armazenamento com o Explorador de servidores](http://msdn.microsoft.com/library/ff683677.aspx).
14. Para ver os detalhes de uma única linha, faça duplo clique uma das linhas.

     ![Tabela de rastreio no Explorador de servidores](./media/web-sites-dotnet-troubleshoot-visual-studio/tws-tracetablerow.png)
...
4. No [Portal do Azure](https://portal.azure.com) **definições** painel para a sua aplicação web, clique em **as credenciais de implementação**e, em seguida, introduza um novo nome de utilizador e palavra-passe.
...
5. Na nova janela do browser, aceda ao URL que é apresentado em **FTP hostname** ou **FTPS hostname** no **aplicação Web** painel para a sua aplicação web.
...
Para obter mais informações sobre como utilizar o modo de depuração no Visual Studio, consulte o [depuração no Visual Studio](http://msdn.microsoft.com/library/vstudio/sc65sadd.aspx) tópico do MSDN e [sugestões de depuração com o Visual Studio 2010](http://weblogs.asp.net/scottgu/archive/2010/08/18/debugging-tips-with-visual-studio-2010.aspx).
...
* [Instruções: Integrar o rastreio de ASP.NET System.Diagnostics rastreio](http://msdn.microsoft.com/library/b0ectfxd.aspx)<br/>
  Isto demasiado é antigo, mas inclui algumas informações adicionais que não abrange o artigo introdutório.
...
Além disso, tenha em atenção que não tem de utilizar o ASP.NET ou System.Diagnostics rastreio se pretende obter de transmissão em fluxo registos do Azure. A aplicação web do Azure, o serviço de registo de transmissão em fluxo irá transmitir qualquer *. txt*, **.HTML*, ou *. log* ficheiro que se encontra no *LogFiles* pasta. Por conseguinte, é possível criar o seu próprio sistema de registo que escreve para o sistema de ficheiros da aplicação web, e o ficheiro será automaticamente transmissão em fluxo e transferido. Tudo o que precisa de executar é escrita código da aplicação que cria ficheiros de *d:\home\logfiles* pasta.
