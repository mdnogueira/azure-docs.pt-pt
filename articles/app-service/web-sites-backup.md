...
Serviço de aplicações pode as seguintes informações para uma conta de armazenamento do Azure e o contentor que configurou a sua aplicação para utilizar a cópia de segurança. 
...
* Cópia de segurança de segurança e restaurar a funcionalidade requer que o plano de serviço de aplicações de ser o **padrão** camada ou **Premium** camada. Para obter mais informações sobre como dimensionar o seu plano do App Service para utilizar um escalão superior, consulte [aumentar verticalmente a uma aplicação no Azure](web-sites-scale.md).  
  **Premium** camada permite um maior número de diariamente fazer uma cópia ups que **padrão** camada.
* Precisa de uma conta de armazenamento do Azure e um contentor na mesma subscrição que a aplicação que pretende efetuar cópia de segurança. Para obter mais informações sobre contas do storage do Azure, consulte o [ligações](#moreaboutstorage) no final deste artigo.
...
1. No [Portal do Azure](https://portal.azure.com), navegue até ao painel da sua aplicação, selecione **cópias de segurança**. O **cópias de segurança** é apresentado o painel.
   
    ![Página de cópias de segurança][ChooseBackupsPage]
   
   > [!NOTE]
   > Se vir a mensagem abaixo, clique na mesma para atualizar o seu plano de serviço de aplicações antes de poder continuar com cópias de segurança.
   > Consulte [aumentar verticalmente a uma aplicação no Azure](web-sites-scale.md) para obter mais informações.  
   > ![Escolha a conta de armazenamento](./media/web-sites-backup/01UpgradePlan1.png)
   > 
   > 

2. No **cópia de segurança** painel, clique em **configurar**
![clique em configurar](./media/web-sites-backup/ClickConfigure1.png)
3. No **configuração de cópia de segurança** painel, clique em **armazenamento: não configurado** para configurar uma conta de armazenamento.
   
    ![Escolha a conta de armazenamento][ChooseStorageAccount]
4. Escolha o destino de cópia de segurança, selecionando um **conta de armazenamento** e **contentor**. A conta de armazenamento têm de pertencer à mesma subscrição que a aplicação que pretende criar cópias de segurança. Se assim o desejar, pode criar uma nova conta de armazenamento ou um novo contentor no respetivos painéis. Quando tiver terminado, clique em **selecione**.
   
    ![Escolha a conta de armazenamento](./media/web-sites-backup/02ChooseStorageAccount1-1.png)
5. No **configuração de cópia de segurança** painel que ainda está deixada aberta, pode configurar **base de dados de cópia de segurança**, em seguida, selecione as bases de dados que pretende incluir nas cópias de segurança (base de dados SQL ou MySQL), em seguida, clique em **OK**.  
   
    ![Escolha a conta de armazenamento](./media/web-sites-backup/03ConfigureDatabase1.png)
   
   > [!NOTE]
   > Para uma base de dados for apresentada nesta lista, a cadeia de ligação tem de existir o **cadeias de ligação** secção o **definições da aplicação** painel para a sua aplicação.
   > 
   > 
6. No **configuração de cópia de segurança** painel, clique em **guardar**.    
7. No **cópias de segurança** painel, clique em **cópia de segurança**.
...
1. No **configuração de cópia de segurança** painel, defina **cópia de segurança agendada** para **no**. 
...
* A aplicação tiver mais de 10 GB de conteúdo (que é a quantidade máxima que pode efetuar cópias de segurança num momento).
* Não pretende que os ficheiros de registo de cópia de segurança.

As cópias de segurança parciais permite escolher exatamente quais os ficheiros pretende para a cópia de segurança.
...
Crie um ficheiro chamado `_backup.filter` e colocar na lista acima no ficheiro, mas remover `D:\home`. Liste um diretório ou ficheiro por linha. Por isso, o conteúdo do ficheiro deve ser:
 ```bash
