<properties
    pageTitle="Replicar máquinas virtuais Hyper-V em nuvens VMM para o Azure utilizando o Azure Site Recovery com o Portal do Azure | Microsoft Azure"
...
- [ARM do PowerShell](site-recovery-vmm-to-azure-powershell-resource-manager.md)
...
> ) diferentes para criar e trabalhar com recursos: o Azure Resource Manager (ARM) e o clássico. O Azure também tem dois portais – o Portal Clássico do Azure que suporta o modelo de implementação clássica, e o Portal do Azure com suporte para ambos os modelos de implementação. 


O Azure Site Recovery no Portal do Azure fornece um número de novas funcionalidades:

- No Portal do Azure, os serviços de Cópia de Segurança do Azure e os do Azure Site Recovery são combinados num único cofre de Serviços de Recuperação, para que possa configurar e gerir a continuidade do negócio e a recuperação após desastre (BCDR) a partir de uma única localização. Um dashboard unificado permite monitorizar e gerir operações em todos os sites no local e nuvem pública do Azure.
...
- A Recuperação de Sites no portal do Azure pode replicar máquinas para contas de armazenamento ARM. Na ativação pós-falha, a Recuperação de Sites cria baseado em VMs baseadas em ARM no Azure.
- A Recuperação de Sites continua a suportar a replicação para as contas de armazenamento clássico. Na ativação pós-falha, a Recuperação de Sites cria VMs utilizando o modelo clássico.


Depois de ler este artigo publique quaisquer comentários na parte inferior dos comentários Disqus. Coloque questões técnicas no [Fórum de Serviços de Recuperação do Azure](https://social.msdn.microsoft.com/forums/azure/home?forum=hypervrecovmgr).
...
- Pode executar facilmente ativações pós-falha a partir da infraestrutura no local para o Azure e reativações pós-falha (recuperação) a partir do Azure para servidores anfitrião Hyper-V no site no local. 
...
--- | ---
**Conta do Azure **| Precisará de uma conta do [Microsoft Azure](http://azure.microsoft.com/). Pode começar com uma [avaliação gratuita](https://azure.microsoft.com/pricing/free-trial/). [Saiba mais](https://azure.microsoft.com/pricing/details/site-recovery/) sobre os preços da Recuperação de Sites. 
**Storage do Azure** | Precisará de uma conta de armazenamento do Azure Standard para armazenar dados replicados. Pode utilizar uma conta de armazenamento LRS ou GRS. Recomendamos GRS para que os dados sejam resilientes se ocorrer uma falha regional ou se a região primária não pode ser recuperada. [Saiba mais](../storage/storage-redundancy.md). A conta tem de ser na mesma região que o cofre de Serviços de Recuperação.<br/><br/>Não há suporte para o armazenamento Premium.<br/><br/> Os dados replicados são guardados no armazenamento do Azure e as VMs do Azure são criadas quando ocorre a ativação pós-falha. <br/><br/> [Leia sobre](../storage/storage-introduction.md) o armazenamento do Azure.
**Rede do Azure** | Precisará de uma rede virtual do Azure à qual se ligarão as VMs do Azure quando ocorrer a ativação pós-falha. A rede tem de estar na mesma região que o cofre de Serviços de Recuperação.
...
**Fornecedor e agente** | Durante a implementação do Azure Site Recovery, instale o Fornecedor do Azure Site Recovery no servidor VMM e o agente dos Serviços de Recuperação nos anfitriões Hyper-V. O Fornecedor e o agente têm de ligar ao Azure através da Internet diretamente ou através de um proxy. Tenha em atenção que um proxy baseado em HTTPS não é suportado. O servidor de proxy no servidor VMM e anfitriões Hyper-V deve permitir acesso a: <br/><br/> *.hypervrecoverymanager.windowsazure.com <br/><br/> *.accesscontrol.windows.net <br/><br/> *.backup.windowsazure.com <br/><br/> *.blob.core.windows.net <br/><br/> *.store.core.windows.net<br/><br/>Se tiver regras de firewall baseadas no endereço IP no servidor VMM, verifique que as regras permitem a comunicação com o Azure. Terá de permitir os endereços IP descritos nos [Intervalos de IP do Datacenter do Azure](https://www.microsoft.com/download/confirmation.aspx?id=41653) e o protocolo HTTPS (433).<br/><br/>Permitir intervalos de endereços IP para a região do Azure da sua subscrição e para EUA Oeste.<br/><br/>Além do servidor de proxy no servidor VMM será necessário aceder a https://www.msftncsi.com/ncsi.txt
...
Precisa de uma rede do Azure para que as VMs do Azure criadas após ativação pós-falha lhe sejam ligadas.

- A rede deve estar na mesma região que aquela na qual implementará o cofre do Serviços de Recuperação.
- Consoante o modelo de recursos que pretende utilizar para efetuar a ativação pós-falha nas VMs do Azure, configure a rede Azure no [modo ARM](../virtual-network/virtual-networks-create-vnet-arm-pportal.md) ou no [modo clássico](../virtual-network/virtual-networks-create-vnet-classic-pportal.md).
- Recomendamos que configure uma rede antes de começar. Se não o fizer terá de fazê-lo durante a implementação da Recuperação de Sites.
...
- Consoante o modelo de recursos que pretende utilizar para efetuar a ativação pós-falha nas VMs do Azure, configure uma conta no [modo ARM](../storage/storage-create-storage-account.md) ou no [modo clássico](../storage/storage-create-storage-account-classic-portal.md).
- Recomendamos que configure uma conta antes de começar. Se não o fizer terá de fazê-lo durante a implementação da Recuperação de Sites.
...
- Durante a implementação da Recuperação de Sites pode especificar que todas as nuvens num servidor VMM devem estar disponíveis no portal do Azure. Se pretender que apenas determinadas nuvens sejam apresentadas no portal, pode ativar essa definição na nuvem na consola do administrador VMM. 
...
5. Transfira a chave de registo. Precisará disto quando executar a configuração. A chave é válida durante 5 dias depois de gerá-la. 
...
- **/Credentials**: é o parâmetro obrigatório que especifica onde se encontra o ficheiro de chave de registo  
...
- **/proxyUsername**: é o parâmetro opcional que especifica o nome de utilizador de proxy (caso o proxy precise de autenticação).
- **/proxyPassword**: é o parâmetro opcional que especifica a palavra-passe para se autenticar com o servidor proxy (caso o proxy precise de autenticação).
...
4. Após a conclusão da instalação clique no botão **Fechar** para concluir.
...
O agente dos Serviços de Recuperação em execução em anfitriões Hyper-V tem acesso à Internet para o Azure para a replicação de VM. Se está a aceder à Internet através de um proxy, configure da seguinte forma:
...
4.  Se ainda não criou uma conta de armazenamento e pretender criar uma com ARM, clique **+ Contas de armazenamento** para fazê-lo de forma embutida.  No painel **Criar conta de armazenamento**, especifique um nome de conta, o tipo, a subscrição e a localização. A conta tem de ter a mesma localização que o cofre de Serviços de Recuperação.

    ![Armazenamento](./media/site-recovery-vmm-to-azure/gs-createstorage.png) 

    Tenha em atenção que:

    - Se pretender criar uma conta de armazenamento utilizando o modelo clássico, vai fazê-lo no portal do Azure. [Saiba mais](../storage/storage-create-storage-account-classic-portal.md)
...
- [Leia](#prepare-for-network-mapping) uma rápida descrição geral do que faz o mapeamento da rede. [Ler este artigo](site-recovery-network-mapping.md) para obter uma explicação mais aprofundada.
- Certifique-se de que as máquinas virtuais no servidor VMM estão ligadas a uma rede VM e que tenha criado pelo menos uma rede virtual do Azure. Tenha em atenção que várias redes VM podem ser mapeadas para uma única rede Azure.
...
5. Em **Rede de destino**, selecione a rede do Azure na qual as VMs da réplica do Azure estarão localizadas quando estes forem aceleradas. Em seguida, clique em **OK**.
...
- As VMs existentes na rede VM de origem serão ligadas à rede de destino quando começa o mapeamento. As novas VMs ligadas à rede VM de origem serão ligadas à rede Azure mapeada quando ocorre a replicação. 
...
3. Em **Copiar frequência**, especifique a frequência com que pretende replicar dados delta após a replicação inicial (cada 30 segundos, 5 ou 15 minutos).
...
12. Em **Definições de replicação** > **Configurar as definições de replicação**, selecione a política de replicação que pretende aplicar para as máquinas virtuais protegidas. Em seguida, clique em **OK**. Pode modificar a política de replicação em **Definições** > **Políticas de replicação** > nome da política > **Editar Definições**. As alterações que aplicar serão utilizadas para as máquinas que já estão a replicar e para as novas máquinas.
