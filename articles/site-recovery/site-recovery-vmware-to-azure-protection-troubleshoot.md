...
Para a maioria dos casos, pode Self-resolver estes problemas, seguindo os passos listados abaixo.
...
Se não siga os passos listados abaixo:
...
Se não, em seguida, aceda ao painel de controlo > serviços, verifique se os seguintes serviços estão em execução:
     ...
     * tmansvc
     * 
(Re) Inicie a qualquer serviço que não está em execução e verifique se o problema ainda existe.
...
* **Verifique se firewall baseada no URL no servidor de processos não está a bloquear o acesso**: Se estiver a utilizar as regras de firewall um URL com base no servidor, certifique-se os seguintes URLs são adicionados à configuração da firewall. 
...
Se precisar de mais ajuda, em seguida, publicar a sua consulta [fórum de ASR](https://social.msdn.microsoft.com/Forums/azure/home?forum=hypervrecovmgr). Temos uma Comunidade de Active Directory e um dos nossos engenheiros conseguirá ajudá-lo.