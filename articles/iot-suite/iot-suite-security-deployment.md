...
A utilização de um [certificado baseado nos dispositivos de x. 509] [ lnk-protocols] e respetivos associado pública e privada par de chaves permite autenticação adicional na camada física. A chave privada é armazenada em segurança no dispositivo e não é detetável fora do dispositivo. O certificado x. 509 contém informações sobre o dispositivo, tal como ID de dispositivo e outros detalhes organizacionais. Uma assinatura do certificado é gerada utilizando a chave privada.
