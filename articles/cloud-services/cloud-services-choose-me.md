...
Como sugere a ilustração anterior, todas as VMs numa única aplicação é executado no mesmo serviço em nuvem. Os utilizadores acedam a aplicação através de um único endereço IP público, com pedidos de carga equilibrada nos VMs da aplicação. A plataforma [dimensiona e implementa](cloud-services-how-to-scale.md) as VMs numa aplicação de serviços em nuvem de uma forma que evita a um ponto único de falha de hardware.
