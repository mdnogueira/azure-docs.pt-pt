---
title: "Resolver problemas de implementação do StorSimple | Microsoft Docs"
...
* Verificar os pré-requisitos para a implementação. Certifique-se de que todas as informações descritas o [lista de verificação de configuração de implementação](storsimple-deployment-walkthrough.md#deployment-configuration-checklist).
...
Durante a implementação do dispositivo, as mais comuns problemas que os utilizadores enfrentam ocorrem quando executam o Assistente de configuração e, quando estes registar o dispositivo através do Windows PowerShell para StorSimple. (Utilizar o Windows PowerShell para StorSimple para registar e configurar o dispositivo StorSimple. Para obter mais informações sobre o registo de dispositivos, consulte [passo 3: configurar e registar o dispositivo através do Windows PowerShell para StorSimple](storsimple-deployment-walkthrough.md#step-3-configure-and-register-the-device-through-windows-powershell-for-storsimple)).
...
Os passos seguintes resumem o processo do Assistente de configuração. Para informações de configuração detalhadas, consulte [implementar o dispositivo StorSimple no local](storsimple-deployment-walkthrough.md).
...
3. Configurar as palavras-passe: o próximo passo é configurar o administrador do dispositivo e as palavras-passe do Snapshot Manager do StorSimple. Se estiver a executar o Update 1, em seguida, não terá de configurar a palavra-passe do Snapshot Manager do StorSimple.
   ...
   * A palavra-passe do Snapshot Manager do StorSimple é necessária quando configura um dispositivo para utilizar o Snapshot Manager do StorSimple. Tem de definir primeiro a palavra-passe no Assistente de configuração e, em seguida, pode definir e alterá-la a partir do serviço StorSimple Manager. Esta palavra-passe autentica o dispositivo com o Snapshot Manager do StorSimple.
     ...
     > 
     > 
4. Registar o dispositivo: é o último passo para registar o dispositivo com o serviço StorSimple Manager em execução no Microsoft Azure. O registo requer que [obter a chave de registo do serviço](storsimple-manage-service.md#get-the-service-registration-key) do portal clássico do Azure e forneça-la no Assistente de configuração. Depois do dispositivo está registado com êxito, uma chave de encriptação de dados do serviço é fornecida para si. Lembre-se de que manter esta chave de encriptação numa localização segura porque será necessária para registar todos os dispositivos subsequentes com o serviço.
...
* Configure o administrador do dispositivo e as palavras-passe do Snapshot Manager do StorSimple. 
...
| 7 |Invocar HcsSetupWizard: Não existem não existem pontos finais mais disponível a partir do mapeador de pontos finais. (Excepção de HRESULT: 0x800706D9) |A funcionalidade de cluster não está a funcionar. |[Contacte o Support Microsoft](storsimple-contact-microsoft-support.md) para passos seguintes. |
...
| 2 |Invocar HcsSetupWizard: Servidor de RPC não está disponível (excepção de HRESULT: 0x800706ba) |A causa raiz é um dos seguintes:<ol><li>O cluster não está até.</li><li>O controlador passivo não é possível comunicar com o controlador de Active Directory e o comando é executado a partir do controlador passiva.</li></ol> |Dependendo da causa raiz:<ol><li>[Contacte o Support Microsoft](storsimple-contact-microsoft-support.md) para se certificar de que o cluster está operacional.</li><li>Execute o comando a partir do controlador ativo. Se pretender executar o comando a partir do controlador de passivo, terá de Certifique-se de que o controlador passivo consegue comunicar com o controlador de Active Directory. Terá [contacte a Microsoft Support](storsimple-contact-microsoft-support.md) se este conectividade está danificada.</li></ol> |
| 3 |HcsSetupWizard invocar: Falha de chamada RPC (excepção de HRESULT: 0x800706be) |Cluster está inativo. |[Contacte o Support Microsoft](storsimple-contact-microsoft-support.md) para se certificar de que o cluster está operacional. |
| 4 |Invocar HcsSetupWizard: Não foi encontrado do recurso de Cluster (excepção de HRESULT: 0x8007138f) |O recurso de cluster não foi encontrado. Isto pode acontecer quando a instalação não estava correta. |Poderá ter de repor o dispositivo para as predefinições de fábrica. [Contacte o Support Microsoft](storsimple-contact-microsoft-support.md) para criar um recurso de cluster. |
...
## <a name="errors-related-to-device-administrator-and-storsimple-snapshot-manager-passwords"></a>Erros relacionados com o administrador do dispositivo e as palavras-passe do Snapshot Manager do StorSimple
...
Se utilizar o software Snapshot Manager do StorSimple em execução no anfitrião do servidor do Windows para gerir o dispositivo, em seguida, tem também de fornecer uma palavra-passe do Snapshot Manager do StorSimple durante o registo da primeira vez. 
...
Além disso, tenha em atenção que as palavras-passe expirarem todos os anos e podem ser alteradas apenas depois de registar o dispositivo com êxito. Se o registo falhar por algum motivo, as palavras-passe não serão alteradas. Para obter mais informações sobre o administrador do dispositivo e as palavras-passe do Snapshot Manager do StorSimple, aceda a [utilizar o serviço StorSimple Manager para alterar as palavras-passe do StorSimple](storsimple-change-passwords.md).
...
| --- | --- | --- |
| 1 |A palavra-passe excede o comprimento máximo. |Utilize uma palavra-passe que cumpra estes requisitos:<ul><li>A palavra-passe de administrador do dispositivo tem de ter entre 8 e 15 carateres.</li><li>A palavra-passe do Snapshot Manager do StorSimple tem de ser entre 14 e 15 carateres.</li></ul> |
| 2 |A palavra-passe não cumpre o comprimento necessário. |Utilize uma palavra-passe que cumpra estes requisitos:<ul><li>A palavra-passe de administrador do dispositivo tem de ter entre 8 e 15 carateres.</li><li>A palavra-passe do Snapshot Manager do StorSimple tem de ser entre 14 e 15 carateres.</lu></ul> |
...
> [!IMPORTANT]
> Em geral, se uma tentativa para aplicar uma palavra-passe falhar, em seguida, o software repetidamente tenta recolher a palavra-passe até ter êxito. Em situações raras, não é possível aplicar a palavra-passe. Nesta situação, pode registar o dispositivo e prosseguir, no entanto as palavras-passe não serão alteradas. Não receberá nenhuma indicação relativamente a que a palavra-passe não foi alterada — a palavra-passe de administrador do dispositivo ou a palavra-passe do Snapshot Manager do StorSimple. Se esta situação ocorrer, recomendamos que altere ambas as palavras-passe.
> 
> 

Só pode repor as palavras-passe no portal clássico do Azure através do serviço StorSimple Manager. Para obter mais informações, aceda a: 
...
* [Alterar a palavra-passe do Snapshot Manager do StorSimple](storsimple-change-passwords.md#change-the-storsimple-snapshot-manager-password).

## <a name="errors-during-device-registration"></a>Erros durante o registo de dispositivos
Pode utilizar o serviço StorSimple Manager em execução no Microsoft Azure para registar o dispositivo. Foi possível encontrar um ou mais dos seguintes problemas durante o registo do dispositivo.

| Não. | Mensagem de erro | Causas possíveis | Ação recomendada |
| --- | --- | --- | --- |
| 1 |Erro 350027: Falha ao registar o dispositivo com o StorSimple Manager. | |Aguarde alguns minutos e, em seguida, repita a operação. Se o problema persistir, [contacte a Microsoft Support](storsimple-contact-microsoft-support.md). |
| 2 |Erro 350013: Ocorreu um erro no registo do dispositivo. Isto pode dever-se a chave de registo do serviço incorreto. | |Registe novamente o dispositivo com a chave de registo do serviço correta. Para obter mais informações, consulte [obter a chave de registo do serviço.](storsimple-manage-service.md#get-the-service-registration-key) |
| 3 |Erro 350063: Falha na autenticação do serviço StorSimple Manager transmitido mas o registo. Repita a operação após algum tempo. |Este erro indica que a autenticação com ACS foi efectuada com êxito, mas falhou a chamada de registo efetuada ao serviço. Isto pode ser um resultado de uma falha de rede esporádicas. |Se o problema persistir,. [contacte a Microsoft Support](storsimple-contact-microsoft-support.md). |
| 4 |Erro 350049: Não foi possível aceder ao serviço durante o registo. |Quando é efetuada a chamada para o serviço, é recebeu uma exceção da web. Em alguns casos, isto poderá obter fixo com repetir a operação mais tarde. |Verifique o endereço IP e nome DNS e, em seguida, repita a operação. Se o problema persistir, [contacte Support da Microsoft.](storsimple-contact-microsoft-support.md) |
...
1. Utilizar o Windows PowerShell para StorSimple para gerar um pacote de suporte, conforme descrito em [criar e gerir um pacote de suporte](storsimple-create-manage-support-package.md).
...
3. Utilize esta opção [procedimento passo a passo](storsimple-create-manage-support-package.md#edit-a-support-package) para abrir e desencriptar o pacote de suporte.
...
Se estiver a executar atualização 1 no dispositivo StorSimple, também estão disponíveis os seguintes cmdlets de diagnóstico.
...
2. Utilizar o resultado do `Get-NetAdapter` cmdlet e as diretrizes seguintes para compreender o estado da sua interface de rede.
   ...
   * Se a interface não existe, não aparecer nesta lista. O serviço StorSimple Manager IU ainda aparecer nesta interface em estado de falha.
...
Utilize o `Test-HcsmConnection` cmdlet para um dispositivo que já está ligado à e registado no serviço StorSimple Manager. Este cmdlet ajuda-o a verificar a conectividade entre um dispositivo registado e o serviço StorSimple Manager correspondente. Pode executar este comando no Windows PowerShell para StorSimple. 
...
3. Certifique-se de que o serviço StorSimple Manager está em execução (utilizar o [Get-ClusterResource](https://technet.microsoft.com/library/ee461004.aspx) cmdlet). Se o serviço de mensagens em fila não está em execução, poderá ver os seguintes erros:
...
5. Verifique a conectividade do serviço de nuvem. Se o serviço emitir uma exceção da web, poderá ver os seguintes erros:
   ...
   * ErrorCode.CiSApplianceACSError – Isto indica que o serviço devolveu um erro de autenticação, mas não existe conectividade.
     ...
     Se não inicia uma exceção da web, verifique a existência de ErrorCode.CiSApplianceSaasServiceError. Isto indica um problema com o serviço StorSimple Manager.
...
O primeiro exemplo é de um dispositivo que está registado com êxito com o serviço StorSimple Manager e tem sem problemas de conectividade. 
...
Se estiver a executar atualização 1 no dispositivo StorSimple, não será necessário executá-lo com o comutador verboso.
...
Este exemplo está a partir de um dispositivo que tem um Estado de **Offline** no portal clássico do Azure.
...
Utilize este cmdlet para apresentar a hora do dispositivo. Se a hora do dispositivo tem um desvio com o servidor NTP, em seguida, pode utilizar este cmdlet força-sincronizar o tempo com o seu servidor NTP. Se o desfasamento entre o dispositivo e o servidor NTP for superior a cinco minutos, irá ver um aviso. Se o desvio excede 15 minutos, o dispositivo será fique offline. Ainda pode utilizar este cmdlet para forçar uma sincronização de hora. No entanto, se o desvio excede 15 horas, em seguida, não será capaz de imposição-sync, que serão apresentadas a hora e uma mensagem de erro.
...
3. Verifique o estado de funcionamento da interface de rede:
   ...
   * Se suspeitar que os dados 0 falhou porta no controlador de Active Directory, pode confirmar isto, ligando-se aos dados porta 0 do controlador 1. Para confirmar isto, desligue o cabo de rede de cópia de segurança do dispositivo do controlador 0, ligue o cabo do controlador 1 e, em seguida, execute o cmdlet Get-NetAdapter novamente. 
     Se os dados 0 porta num controlador de falha, [contacte a Microsoft Support](storsimple-contact-microsoft-support.md) para passos seguintes. Poderá ter de substituir o controlador no seu sistema.
...
5. Eliminar quaisquer erros de utilizador:
   ...
   * Verifique o registo chave utilizada. A mesma chave de registo pode ser utilizada para ligar a vários dispositivos para um serviço StorSimple Manager. Utilize o procedimento no [obter a chave de registo do serviço](storsimple-manage-service.md#get-the-service-registration-key) para se certificar de que está a utilizar a chave de registo correto.
     
     > [!IMPORTANT]
     > Se tiver múltiplos serviços em execução, terá de se certificar de que a chave de registo para o serviço adequada é utilizada para registar o dispositivo. Se registou um dispositivo com o serviço StorSimple Manager errado, precisa de [contacte a Microsoft Support](storsimple-contact-microsoft-support.md) para passos seguintes. Poderá ter de efetuar uma reposição de fábrica do dispositivo (o que poderá resultar na perda de dados) para, em seguida, ligue-o para o serviço que se destinam.
     > 
     > 
...
9. Se os passos anteriores não resolverem o problema, [contacte a Microsoft Support](storsimple-contact-microsoft-support.md) para obter assistência.
