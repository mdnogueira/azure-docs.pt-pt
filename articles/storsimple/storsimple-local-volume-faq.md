...
Em dispositivos com a atualização de série de 8000 do StorSimple 2, pode aprovisionar volumes localmente afixados até 8 TB ou volumes em camadas até 200 TB no dispositivo 8100. No dispositivo 8600 maior, pode aprovisionar volumes localmente afixados até 20 TB ou volumes em camadas até 500 TB.   

**P.** Posso atualizado recentemente o meu dispositivo 8100 para atualização 2.0 e quando tentar criar um volume localmente afixado, o tamanho máximo de disponível é apenas 6 TB e não de 8 TB. Por que motivo não é possível a criar um volume de 8 TB?

**A** se o dispositivo estiver em execução de atualização 2.0, pode aprovisionar volumes localmente afixados até 8 TB ou volumes em camadas até 200 TB no dispositivo 8100. Se o dispositivo já tem volumes em camadas, o espaço disponível para a criação de um volume localmente afixado será proporcionalmente inferior a este limite máximo. Por exemplo, se já tiverem sido aprovisionada 100 TB de volumes em camadas no dispositivo 8100 (que é metade da capacidade em camadas), em seguida, o tamanho máximo de um volume local que pode criar no dispositivo 8100 será proporcionalmente reduzido para 4 TB (aproximadamente metade máximo localmente afixado capacidade do volume).
...
####<a name="update-2x"></a>Atualização 2  
 | Capacidade de aprovisionamento de volumes localmente afixados | Capacidade disponível para ser aprovisionada para volumes em camadas - 8100 | Capacidade disponível para ser aprovisionada para volumes em camadas - 8600 |  
 | --- | --- | --- |  
 | 0 |200 TB |500 TB |  
 | 1 TB |25 TB |475 TB |  
...
* Pode converter os volumes em camadas existentes volumes localmente afixados e esta conversão envolve o aprovisionamento de espaço no dispositivo para o volume localmente afixado resultante (para além de desativar a dados em camadas [se qualquer] da nuvem). Novamente, esta é uma operação de execução longa que depende de fatores que discutimos acima. Sugerimos que copie em segurança os volumes existentes antes da conversão como o processo será ainda mais lento se os volumes existentes não são guardados em cópia de segurança. O dispositivo também pode ocorrer reduzido desempenho durante este processo.
...
**R.** Se eliminar um volume localmente afixado, o espaço disponível para novos volumes não pode ser atualizado imediatamente. O serviço StorSimple Manager atualiza o espaço local disponível aproximadamente a cada hora. Sugerimos que aguarde uma hora antes de tentar criar o novo volume.
...
* Será desencadeado um alerta crítico para notificá-lo da falha na conversão do volume. Obter mais informações sobre [alertas relacionados com volumes localmente afixados](storsimple-manage-alerts.md#locally-pinned-volume-alerts)
...
**R.** Sim, pode. No entanto, o volume localmente afixado será clonado como um volume em camadas por predefinição. Obter mais informações sobre como [clonar um volume localmente afixado](storsimple-clone-volume-u2.md)
...
**R.** Dependendo da versão do software do dispositivo de destino, volumes localmente afixados irão a ativação pós-falha como:

* Afixado localmente se o dispositivo de destino está em execução 8000 do StorSimple série atualização 2
* Em camadas se o dispositivo de destino está em execução StorSimple 8000 série atualização 1. x
* Se o dispositivo de destino for o dispositivo de nuvem de camadas (atualização de versão do software 2 ou atualizar 1. x)

Obter mais informações sobre [ativação pós-falha e DR de afixado localmente volumes entre versões](storsimple-device-failover-disaster-recovery.md#device-failover-across-software-versions)
...
**R.** Não, não é possível alterar o tipo de volume durante uma ativação pós-falha. Se estão a falhar através de outro dispositivo físico que está a executar 8000 do StorSimple série de atualização 2, os volumes serão efetuados a ativação pós-falha com base no tipo de volume armazenado no instantâneo. Quando efetuar a ativação pós-falha para qualquer outra versão do dispositivo, consulte a questão acima no tipo de volume após uma ativação pós-falha.

**P.** Pode falhar ao longo de um contentor de volume com volumes localmente afixados para a aplicação na nuvem?

**R.** Sim, pode. Os volumes localmente afixados irão a ativação pós-falha como volumes em camadas. Obter mais informações sobre [ativação pós-falha e DR de afixado localmente volumes entre versões](storsimple-device-failover-disaster-recovery.md#considerations-for-device-failover)
