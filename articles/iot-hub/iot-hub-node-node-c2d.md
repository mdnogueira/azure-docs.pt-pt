...
IoT Hub do Azure é um serviço completamente gerido que o ajuda a ativar fiáveis e seguras comunicações bidirecionais entre milhões de dispositivos e uma solução de back-end. O [introdução ao IoT Hub] tutorial mostra como:

* Crie um hub IoT.
* Aprovisione uma identidade de dispositivo num IoT hub.
* O código de uma aplicação de dispositivo simulado que envia mensagens do dispositivo para a nuvem.
...
1. Crie uma pasta vazia com o nome `sendcloudtodevicemessage`. Na pasta `sendcloudtodevicemessage`, crie um ficheiro package.json com o comando seguinte na sua linha de comandos. Aceite todas as predefinições:

    ```cmd/sh
    npm init
    ```

1. Na sua linha de comandos a `sendcloudtodevicemessage` pasta, execute o seguinte comando para instalar o **azure iothub** pacote:

    ```cmd/sh
    npm install azure-iothub --save
    ```

1. Com um editor de texto, crie um **SendCloudToDeviceMessage.js** ficheiros o `sendcloudtodevicemessage` pasta.

1. Adicione o seguinte `require` as instruções no início do **SendCloudToDeviceMessage.js** ficheiro:
...
1. Adicione o seguinte código para **SendCloudToDeviceMessage.js** ficheiro. Substitua o `{iot hub connection string}` valor do marcador de posição pela cadeia de ligação do IoT Hub hub que criou no [introdução ao IoT Hub] tutorial. Substitua o `{device id}` marcador de posição com o ID de dispositivo do dispositivo adicionado no [introdução ao IoT Hub] tutorial:
...
1. Adicione a seguinte função para imprimir mensagens de comentários de entrega para a consola:
...
1. Guarde e feche **SendCloudToDeviceMessage.js** ficheiro.
...
1. Numa linha de comandos do `sendcloudtodevicemessage` pasta, execute o seguinte comando para enviar uma mensagem da nuvem para o dispositivo e a aguardar o comentários de confirmação:

    ```cmd/sh
    node SendCloudToDeviceMessage.js
    ```

    ![Para enviar o comando de nuvem para o dispositivo, executar a aplicação][img-send-command]
