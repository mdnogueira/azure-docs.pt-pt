...
* **[Pré-visualização de clusters associados a um domínio](../domain-joined/apache-domain-joined-introduction.md)**: um cluster associado a um domínio do Active Directory, para que possa controlar o acesso e os dados.
...
* [Introdução ao Hadoop no HDInsight](apache-hadoop-linux-tutorial-get-started.md): um tutorial de início rápido para o aprovisionamento de clusters do HDInsight Hadoop e a execução de consultas do Hive de exemplo.
* [Começar com o Spark no HDInsight](../spark/apache-spark-jupyter-spark-sql.md): um tutorial de início rápido para criar um cluster Spark e executar consultas de Spark SQL interativas.
* [Utilizar o R Server no HDInsight](../r-server/r-server-get-started.md): começar a utilizar o R Server no HDInsight Premium.
