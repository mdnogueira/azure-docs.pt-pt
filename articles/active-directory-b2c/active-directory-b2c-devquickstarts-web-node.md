<properties
    pageTitle="Adicionar início de sessão a uma aplicação Web Node.js para pré-visualização do Azure B2C | Microsoft Azure"
...
# Pré-visualização do B2C: Adicionar início de sessão a uma aplicação Web Node.js
...
> [AZURE.WARNING]   Para esta pré-visualização do B2C, tem de utilizar o mesmo **ID de cliente**/**ID da aplicação** e políticas para o servidor de tarefas da API Web e o cliente que se liga ao mesmo. Isto também se aplica aos tutoriais do iOS e Android. Se anteriormente tiver criado uma aplicação em qualquer um desses Inícios Rápidos, utilize esses valores; não crie valores novos.
...
## Adicionar pré-requisitos ao diretório
...
## Configurar a sua aplicação para utilizar a estratégia Passport-Node.js
...
## Utilizar o Passport para emitir pedidos de início de sessão e fim de sessão para o Azure AD
...
- `/logout` chama simplesmente `logout.ejs` (e a respetiva rota). Isto limpa cookies e, em seguida, devolve o utilizador a `index.ejs`.
...
## Criar as vistas e rotas no Express para chamar as políticas
