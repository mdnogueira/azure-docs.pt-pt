...
2. Configure a aplicação para utilizar o plug-in `azure-ad-passport` do Passport.
...
No Azure AD B2C, cada experiência de utilizador é definida por uma [política](active-directory-b2c-reference-policies.md). Esta aplicação contém duas experiências de identidade: registo e início de sessão. Tem de criar uma política de cada tipo, conforme descrito no [artigo de referência de política](active-directory-b2c-reference-policies.md#how-to-create-a-sign-up-policy).  Quando criar as três políticas, não se esqueça de:
...
> [AZURE.NOTE] Esta passagem parte do princípio de que irá utilizar os pontos finais do servidor e a instalação predefinida para o MongoDB, que de momento é `mongodb://localhost`.
...
`IdentityMetadata`: este é o local onde `passport-azure-ad` vai procurar os seus dados de configuração para o fornecedor de identidade. Também vai procurar aqui as chaves para validar os tokens Web JSON. 
...
`completed`: se a tarefa estiver concluída. Este objeto é um **Booleano**.
...
## Execute a aplicação de servidor para verificar se esta o rejeita
...
Na janela de terminal, execute `curl`
