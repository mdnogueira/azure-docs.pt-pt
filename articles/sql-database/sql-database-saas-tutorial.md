...
A implementação inicial cria três inquilinos de exemplo, mas vamos criar outro inquilino para ver como este problema afeta a aplicação implementada. O fluxo de trabalho do inquilinos aprovisionamento de Wingtip SaaS está detalhado no [tutorial de aprovisionamento e o catálogo](sql-database-saas-tutorial-provision-and-catalog.md). Neste passo, criar rapidamente um novo inquilino.
...
* Adicionais [tutoriais que criar na aplicação Wingtip SaaS](sql-database-wtp-overview.md#sql-database-wingtip-saas-tutorials)
