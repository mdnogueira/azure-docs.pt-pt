...
---
# <a name="getting-started-with-azure-sql-data-sync-preview"></a>Introdução à sincronização de dados SQL do Azure (pré-visualização)
...
3.  No **bases de dados SQL** painel, selecione a base de dados existente do SQL Server que pretende utilizar como a base de dados do hub para sincronização de dados. É aberto o painel de base de dados do SQL Server.

4.  No painel de base de dados do SQL Server para a base de dados selecionada, selecione **sincronização para outras bases de dados**. É aberto o painel de sincronização de dados.
...
1.  No painel de sincronização de dados, selecione **novo grupo de sincronização**. O **novo grupo de sincronização** painel abre-se com o passo 1, **criar grupo de sincronização**, realçado. O **criar grupo de sincronização de dados** também é aberto o painel.

2.  No **criar grupo de sincronização de dados** painel, efetue os seguintes procedimentos:
    ...
    2.  No **base de dados de metadados de sincronização** secção, escolha se pretende criar uma nova base de dados (recomendado) ou utilizar uma base de dados existente.
        ...
        Se tiver escolhido **nova base de dados**, selecione **criar nova base de dados.** O **base de dados SQL** abre o painel. No **base de dados SQL** painel, atribua um nome e configure a nova base de dados. Em seguida, selecione **OK**.
...
Depois do novo grupo de sincronização é criado e implementado, o passo 2, **adicionar membros de sincronização**, estiver realçado no **novo grupo de sincronização** painel.
...
No **base de dados do membro** secção, opcionalmente, adicione uma base de dados do SQL do Azure para o grupo de sincronização selecionando **adicionar uma base de dados do Azure**. O **configurar a base de dados do Azure** abre o painel.

No **configurar a base de dados do Azure** painel, efetue os seguintes procedimentos:
...
No **base de dados do membro** secção, opcionalmente, adicione um servidor de SQL no local para o grupo de sincronização selecionando **adicionar uma base de dados do On-Premises**. O **configurar On-Premises** abre o painel.

No **configurar On-Premises** painel, efetue os seguintes procedimentos:

1.  Selecione **escolher o Gateway do agente de sincronização**. O **selecionar agente de sincronização** abre o painel.
...
2.  No **escolher o Gateway do agente de sincronização** painel, escolha se pretende utilizar um agente existente ou criar um novo agente.
    ...
    5.  Selecione **OK** para fechar o **selecionar agente de sincronização** painel.
    ...
    12. No portal, no **configurar On-Premises** painel, selecione **selecione a base de dados.** O **selecionar base de dados** abre o painel.

    13. No **selecionar base de dados** painel, no **nome de membro de sincronização** campo, forneça um nome para o novo membro de sincronização. Este nome é diferente do nome da base de dados própria. Selecione a base de dados da lista. No **indicações de sincronização** campo, selecione sincronização bidirecional, para o Hub ou do Hub.

        ![Selecione a base de dados no local](media/sql-database-get-started-sql-data-sync/datasync-preview-selectdb.png)

    14. Selecione **OK** para fechar o **selecionar base de dados** painel. Em seguida, selecione **OK** para fechar o **configurar On-Premises** painel e aguarde que o novo membro de sincronização ser criada e implementada. Por fim, clique em **OK** para fechar o **selecionar membros de sincronização** painel.
...
Depois dos novos membros do grupo de sincronização são criados e implementados, passo 3, **configurar grupo de sincronização**, estiver realçado no **novo grupo de sincronização** painel.

1.  No **tabelas** painel, selecione uma base de dados da lista de sincronizar os membros do grupo e, em seguida, selecione **atualizar esquema**.
