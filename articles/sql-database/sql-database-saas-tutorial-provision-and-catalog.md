...
Para além da aplicação SaaS, o catálogo pode ativar as ferramentas de base de dados.  No exemplo Wingtip SaaS, o catálogo é utilizado para ativar consulta de inquilino em vários locais, explorou no [tutorial análise ad hoc](sql-database-saas-tutorial-adhoc-analytics.md). Gestão de tarefas entre bases de dados é explorou no [gestão esquema](sql-database-saas-tutorial-schema-management.md) e [inquilino análise](sql-database-saas-tutorial-tenant-analytics.md) tutoriais. 
...
Depois de ter concluído o aprovisionamento, execução devolve para original *demonstração ProvisionAndCatalog* script, que abre o **eventos** página para o novo inquilino no browser:
   ...
   ![eventos](media/sql-database-saas-tutorial-provision-and-catalog/new-tenant.png)
...
Repita o [tutorial de monitorização de desempenho](sql-database-saas-tutorial-performance-monitoring.md).
