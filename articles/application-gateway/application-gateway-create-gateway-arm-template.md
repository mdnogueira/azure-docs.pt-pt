...
- Configurar dois IPs de back-end configurados anteriormente para os servidores Web para os quais pretende balancear o tráfego. Neste exemplo de modelo, os IPs de back-end serão 10.0.1.10 e 10.0.1.11.

>[AZURE.NOTE] Trata-se dos parâmetros para este modelo. Para personalizar o modelo, pode alterar as regras, o serviço de escuta e o SSL que abre o azuredeploy.json.
...
5. Abra o ficheiro que guardou e observe o conteúdo em **parâmetros**, na linha 5. Os parâmetros do modelo Azure Resource Manager fornecem um marcador de posição para valores que podem ser preenchidos durante a implementação.
  	...
  	| Parâmetro | Descrição |
  	|---|---|
  	| **localização** | Região do Azure onde será criado o gateway de aplicação |
...
Se nunca tiver utilizado o Azure PowerShell, veja [How to install and configure Azure PowerShell (Como instalar e configurar o Azure PowerShell)](../powershell-install-configure.md) e siga as instruções até ao fim para iniciar sessão no Azure e selecionar a sua subscrição.
...
Se necessário, crie um novo grupo de recursos com o cmdlet **New-AzureResourceGroup**. No exemplo abaixo, irá criar um novo grupo de recursos denominado AppgatewayRG na localização EUA Leste.
...
Execute o cmdlet **New-AzureRmResourceGroupDeployment** para implementar a nova rede virtual com o modelo e os ficheiros de parâmetros transferidos e alterados acima.
...
**-l (ou --location)**. Região do Azure onde será criado o novo grupo de recursos. Para o nosso cenário, a região é *eastus*.

4. Execute o cmdlet **azure group deployment create** para implementar a nova rede virtual com o modelo e os ficheiros de parâmetros que transferiu e alterou acima. A lista apresentada depois do resultado explica os parâmetros utilizados.
...
**-g (ou --resource-group)**. Nome do grupo de recursos onde será criada a nova rede virtual.
