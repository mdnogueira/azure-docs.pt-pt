...
8. Feche a aplicação.  
   ...
   >[AZURE.NOTE] Para receber um alerta de notificação push, a aplicação não pode estar a ser executada em primeiro plano.

##Enviar notificações push a partir do back-end
...
Para obter um exemplo de como enviar notificações push de um back-end de ASP.NET WebAPI que esteja integrada com Notification Hubs, consulte o artigo [Os Notification Hubs do Azure Notificam os Utilizadores com back-end .NET](./notification-hubs-aspnet-backend-windows-dotnet-notify-users.md).  

Para obter um exemplo sobre como enviar notificações push utilizando as [APIs REST](https://msdn.microsoft.com/library/azure/dn223264.aspx), consulte [Como utilizar os Notification Hubs de Java](./notification-hubs-java-backend-how-to.md) e [Como utilizar os Notification Hubs de PHP](./notification-hubs-php-backend-how-to.md).
...
1. Clique com o botão direito do rato na solução, selecione **Adicionar** e **Novo Projeto…**, e, em seguida, em **Visual C#**, clique em **Windows** e **Aplicação de Consola** e, em seguida, clique em **OK**.
    ...
    ![Visual Studio – Novo Projeto – Aplicação de Consola][6]

    Esta ação adiciona uma nova aplicação da consola Visual C# à solução. Também pode fazer isto numa solução separada.

4. Clique em **Ferramentas**, clique em **Gestor de Pacotes de Biblioteca** e, em seguida, em **Consola do Gestor de Pacotes**.

    Esta ação exibe a Consola do Gestor de Pacotes.

5.  Na janela **Consola do Gestor de Pacotes**, defina o **Projeto Predefinido** como o seu novo projeto da aplicação de consola e, em seguida, execute o seguinte comando na janela da consola:
...
6. Abra o ficheiro `Program.cs` e adicione a seguinte instrução `using`:

        using Microsoft.Azure.NotificationHubs;

6. Na classe `Program`, adicione o método seguinte:
