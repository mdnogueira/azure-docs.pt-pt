...
---
# <a name="examples-of-mapping-an-existing-web-service-to-odata-through-csdls"></a>Exemplos de mapeamento de um serviço web existente para OData através de CSDLs
> [!IMPORTANT]
...
## <a name="example-functionimport-for-raw-data-returned-using-post"></a>Exemplo: FunctionImport para dados de "Raw" devolvidos utilizando "POST"
Utilize dados não processados POST para criar um novo subordinado e devolver o respetivo servidor definido URL(location) ou ao atualizar a parte do subordinado no servidor definido URL.  Em que o subordinado é um fluxo, ou seja, não estruturado, por ex. um ficheiro de texto.  Cuidado com POST na não idempotent sem uma localização.
...
## <a name="example-functionimport-using-delete"></a>Exemplo: FunctionImport utilizar "Eliminar"
Utilize a eliminar para remover um URI especificado.
...
## <a name="example-functionimport-using-post"></a>Exemplo: FunctionImport utilizar "Publicar"
Utilize dados não processados POST para criar um novo subordinado e devolver o respetivo servidor definido URL(location) ou ao atualizar a parte do subordinado no servidor definido URL.  Onde o subordinado é uma estrutura. Cuidado com POST não é idempotent sem uma localização.
...
## <a name="example-functionimport-using-put"></a>Exemplo: FunctionImport utilizar a "Colocar"
Utilize o PUT para criar um novo subordinado ou para atualizar o subordinado de todo um URL de servidor definido.  Em que o subordinado é uma estrutura, PUT é idempotent para as múltiplas ocorrências irão resultar no mesmo Estado, revertidos x = 5.  PUT deve ser utilizada com o conteúdo completo o recurso especificado.
...
## <a name="example-functionimport-for-raw-data-returned-using-put"></a>Exemplo: FunctionImport para dados de "Raw" devolvidos utilizando a "Colocar"
Utilize dados colocar não processados para criar um novo subordinado ou para atualizar o subordinado de todo um URL de servidor definido.  Em que o subordinado é um fluxo, ou seja, não estruturado, por ex. um ficheiro de texto.  PUT é idempotent para as múltiplas ocorrências irão resultar no mesmo Estado, revertidos x = 5.  PUT deve ser utilizada com o conteúdo completo o recurso especificado.
...
## <a name="example-functionimport-for-raw-data-returned-using-get"></a>Exemplo: FunctionImport para dados de "Raw" devolvidos utilizando "GET"
Utilize dados obter não processado para devolver subordinado que é não estruturado, ou seja, o texto.
...
## <a name="example-functionimport-for-paging-through-returned-data"></a>Exemplo: FunctionImport para "Paginação" através de dados devolvidos
Utilize a paginação RESTful de implementar através do seus dados com GET.  A paginação de predefinição está definida para 100 linha por página de dados.
