<properties
    pageTitle="O suporte de CORS no App Service | Microsoft Azure"
...
## <a id="corsconfig"></a> Como configurar a CORS no App Service do Azure

Pode configurar a CORS no Portal do Azure ou através das ferramentas do [Azure Resource Manager](../resource-group-overview.md).

#### Configurar a CORS no Portal do Azure

8. Num browser, aceda ao [Portal do Azure](https://portal.azure.com/).
...
10. No painel **Definições** que abre à direita do painel **Aplicação API**, localize a secção **API** e, em seguida, clique em **CORS**.

    ![Selecionar a CORS no painel Definições](./media/app-service-api-cors-consume-javascript/clicksettings.png)

11. Na caixa de texto introduza o URL ou URLs a partir dos quais pretende receber chamadas JavaScript.
...
#### Configurar a CORS utilizando ferramentas do Azure Resource Manager
...
## <a id="tutorialstart"></a> Continuar o tutorial de introdução ao .NET
...
## Implementar o projeto ToDoListAngular numa nova aplicação Web
...
### Criar uma nova aplicação Web para o projeto ToDoListAngular
...
3. No separador **Alojamento** da caixa de diálogo **Criar App Service**, introduza um **Nome da Aplicação Web** que seja exclusivo no domínio *azurewebsites.net*. 
...
4. Na lista pendente **Plano do App Service**, selecione o mesmo plano que criou anteriormente. 
...
### Definir o URL de camada média nas definições da aplicação Web
...
### Implementar o projeto Web ToDoListAngular na nova aplicação Web
...
### Testar a aplicação sem a CORS ativada 

2. Nas Ferramentas de Programação do seu browser, abra a janela Consola.

3. Na janela do browser que apresenta a IU do AngularJS, clique na ligação **Lista de Tarefas**.
...
## Configurar a CORS para a aplicação API de camada média
...
12. Na caixa de texto, introduza o URL para a aplicação Web ToDoListAngular (front-end). Por exemplo, se tiver implementado o projeto ToDoListAngular numa aplicação Web com o nome todolistangular0121, permita chamadas a partir do URL `https://todolistangular0121.azurewebsites.net`.
...
### Testar a aplicação com a CORS ativada
...
## CORS do App Service versus CORS da API Web
...
1. Inclua uma `config.EnableCors()` linha de código no método **Registar** da classe **WebApiConfig**, como no exemplo seguinte. 
...
1. No controlador da API Web, adicione uma declaração `using` para o espaço de nomes `System.Web.Http.Cors` e adicione o atributo `EnableCors` para a classe do controlador ou métodos de ação individuais. No exemplo seguinte, o suporte de CORS aplica-se ao controlador na totalidade.
...
## Utilizar a API Management do Azure com as API Apps
...
* [Descrição Geral da API Management do Azure (vídeo: CORS começa às 12:10)](https://azure.microsoft.com/documentation/videos/azure-api-management-overview/)
* [Políticas entre domínios da API Management](https://msdn.microsoft.com/library/azure/dn894084.aspx#CORS)
