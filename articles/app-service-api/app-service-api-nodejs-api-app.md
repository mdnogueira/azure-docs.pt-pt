<properties
    pageTitle="Aplicação API Node.js no App Service do Azure | Microsoft Azure"
...
1. [Node.js](http://nodejs.org) instalado (este exemplo pressupõe que tem a versão 4.2.2 do Node.js)
...
1. Navegue para uma pasta que possa utilizar para um repositório local do Git e clone o [repositório GitHub que contém o código de exemplo](https://github.com/Azure-Samples/app-service-api-node-contact-list).
...
## Estruturar (gerar automaticamente) o código Node.js com base nos metadados Swagger
...
>[AZURE.NOTE] Pode ignorar esta secção se não quiser saber como estruturar o código do Node.js a partir de um ficheiro de metadados Swagger. Se pretender implementar apenas código de exemplo numa nova aplicação API, aceda diretamente à secção [Criar uma aplicação API no Azure](#createapiapp).
...
2. Navegue para a pasta *Iniciar* pasta e, em seguida, execute o comando `yo swaggerize`. O Swaggerize irá fazer uma série de perguntas.  Para **o que chamar a este projeto**, introduza "contactlist", para **caminho para o documento swagger**, introduza "api.json" e para **Express, Hapi ou Restify**, introduza "express".
...
1. Navegue para a pasta que contém o código estruturado (neste caso, a subpasta *ContactList*).
...
### Personalizar o código estruturado

1. Copie a pasta **lib** a partir da pasta **Iniciar** para a pasta **ContactList** criada pelo estruturador. 

1. Substitua o código no ficheiro **handlers/contacts.js** pelo código seguinte. 
...
1. Substitua o código no ficheiro **handlers/contacts/{id}.js** pelo código seguinte. 
...
1. Substitua o código no **server.js** pelo código seguinte. 
...
1. Ao navegar para **http://localhost:8000/contacts**, verá a saída JSON da lista de contactos (ou será avisado para transferi-la, dependendo do seu browser). 

    ![Chamada à Api Todos os Contactos](media/app-service-api-nodejs-api-app/all-contacts-api-call.png)

1. Ao navegar para **http://localhost:8000/contacts/2**, verá o contacto representado por esse valor de id.

    ![Chamada à Api Contacto Específico](media/app-service-api-nodejs-api-app/specific-contact-api-call.png)

1. Os dados Swagger JSON são servidos através do ponto final **/swagger**:

    ![Contactos Swagger Json](media/app-service-api-nodejs-api-app/contacts-swagger-json.png)

1. A IU do Swagger é servida através do ponto final **/docs**. Na IU do Swagger, pode utilizar as funcionalidades de cliente HTML avançadas para testar a sua API.
...
1. Clique em **Novo > Web + Móvel > Aplicação API**. 

    ![Nova aplicação API no portal](media/app-service-api-nodejs-api-app/new-api-app-portal.png)

4. Introduza um **Nome de aplicação** que seja exclusivo no domínio *azurewebsites.net*, tal como NodejsAPIApp e um número para torná-lo único. 
...
5. No painel **Plano do App Service**, introduza "NodejsAPIAppPlan" ou outro nome, se preferir.
...
5. Clique em **Escalão de preço > Ver Tudo > F1 Gratuito**.
...
6. No painel **Plano do App Service**, clique em **OK**.

7. No painel **Aplicação API**, clique em **Criar**.

## Configurar a nova aplicação API para a implementação do Git
...
1. No painel **Definições**, desloque-se para baixo para a secção **Publicação** e, em seguida, clique em **Credenciais de implementação**.
...
1. No painel **Definições**, clique em **Origem da implementação > Escolher Origem > Repositório Local do Git** e, em seguida, clique em **OK**.

    ![Criar Repositório do Git](media/app-service-api-nodejs-api-app/create-git-repo.png)

1. Quando o repositório do Git tiver sido criado, o painel muda para mostrar as implementações ativas. Uma vez que o repositório é novo, não tem implementações ativas na lista. 
...
## Implementar o código API no Azure
...
1. Na ferramenta de linha de comandos, navegue para a nova pasta e, em seguida, execute o seguinte comando para criar um novo repositório de Git local. 

        git init

     ![Novo Repositório de Git Local](media/app-service-api-nodejs-api-app/new-local-git-repo.png)

1. Execute o comando seguinte para adicionar um Git remoto para o repositório da sua Aplicação API. 

        git remote add azure YOUR_GIT_CLONE_URL_HERE

    **Nota**: substitua a cadeia "YOUR_GIT_CLONE_URL_HERE" pelo seu URL de clone de Git que copiou anteriormente. 

1. Execute os seguintes comandos para criar uma consolidação que contenha todo o conteúdo do código. 
...
1. Execute o comando para emitir o código para o Azure. Quando lhe for pedida uma palavra-passe, introduza aquela que criou previamente no Portal do Azure.

        git push azure master

    Isto aciona uma implementação para a sua aplicação API.  

1. No seu browser, navegue de volta para o painel **Implementações** da aplicação API e irá verificar que a implementação está a ocorrer. 
...
## Testar com a API em execução no Azure
 
3. Copie o **URL** na secção **Princípios Básicos** do painel Aplicação API. 

    ![Implementação Concluída](media/app-service-api-nodejs-api-app/deployment-completed.png)

1. Se utilizar um cliente API REST como o Postman ou Fiddler (ou o seu browser), forneça o URL da chamada API dos seus contactos, que é o ponto final `/contacts` da sua aplicação API. O URL será `https://{your API app name}.azurewebsites.net/contacts`
...
2. Num browser, aceda ao ponto final `/docs` para experimentar a IU do Swagger enquanto é executada no Azure.
