...
5. O [Azure Application Insights](../application-insights/app-insights-overview.md) monitoriza a sua aplicação Web quanto à disponibilidade, ao desempenho e á utilização. A caixa de verificação **Adicionar Application Insights ao Projeto** está selecionada por predefinição da primeira vez que criar um projeto Web depois de instalar o Visual Studio. Desmarque a caixa de verificação se estiver selecionada, mas não pretender experimentar o Application Insights.
...
* Elimine o grupo de recursos que contém a aplicação Web e quaisquer recursos relacionados do Azure quando tiver terminado com os mesmos.
    ...
    Para obter informações sobre como trabalhar com grupos de recursos no Portal do Azure, veja [Implementar recursos com modelos do Resource Manager e com o portal do Azure](../azure-resource-manager/resource-group-template-deploy-portal.md).   
