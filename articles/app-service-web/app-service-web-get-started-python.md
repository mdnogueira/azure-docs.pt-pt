<properties 
    pageTitle="Implementar a primeira aplicação Web Python no Azure em cinco minutos | Microsoft Azure" 
...
# Implementar a primeira aplicação Web Python no Azure em cinco minutos
...
4. Implemente o código de exemplo na sua aplicação do Azure tal como emitiria qualquer código com o Git:
    ...
    Se utilizou uma das arquiteturas de linguagem, será apresentado um resultado diferente. Tal deve-se ao facto de que `git push` não só coloca código no Azure, como também aciona tarefas de implementação no motor de implementação. Se tiver algum ficheiro requirements.txt na raiz do projeto (repositório), o script de implementação restaura os pacotes necessários por si. 
