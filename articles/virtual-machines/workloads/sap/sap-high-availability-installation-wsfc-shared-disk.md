---
title: "SAP NetWeaver HA instalação no Cluster de ativação pós-falha do Windows e o disco partilhado para a instância SCS SAP (A) no Azure | Microsoft Docs"
description: "SAP NetWeaver HA instalação no Cluster de ativação pós-falha do Windows e o disco partilhado para a instância SCS SAP (A)"
...
---
# <a name="sap-netweaver-ha-installation-on-windows-failover-cluster-and-shared-disk-for-sap-ascs-instance-on-azure"></a>SAP NetWeaver HA instalação no Cluster de ativação pós-falha do Windows e o disco partilhado para a instância SCS SAP (A) no Azure
...
Este documento é que descrevem como instalar e configurar elevado sistema SAP disponível no Azure, com **Cluster de ativação pós-falha do Windows (WSFC)** e utilizar **disco partilhado de cluster** para clustering SCS SAP (A) de instância.

## <a name="prerequisites"></a>Pré-requisitos

Certifique-se de que revê estes documentos antes de iniciar a instalação:

* [Guia de arquitetura - Clustering SAP (A) SCS instância no Cluster de ativação pós-falha do Windows utilizando o Cluster de disco partilhado][sap-high-availability-guide-wsfc-shared-disk]
...
### <a name="a97ad604-9094-44fe-a364-f89cb39bf097"></a>Criar um nome de anfitrião Virtual para a instância ASCS/SCS SAP em cluster
...
2.  Para manter as predefinições de portas para o Balanceador de carga interno do Azure, selecione:
  ...
  * **Sistema ABAP + Java**: **ASCS** instância número **00** e **SCS** instância número **01**
  ...
  Para utilizar números de instância diferente 00 para a instância de ABAP ASCS e 01 para o Java SCS instância, primeiro tem de alterar o Azure de carga interno balanceador predefinido balanceamento de carga regras, descrito em [alteração regras de balanceamento de carga do predefinido ASCS/SCS para o Azure Balanceador de carga interno] [sap-ha-guia-8.9].
...
1.  Verifique o atual **ProbePort** definição executando o seguinte comando do PowerShell:
  ...
  ```
   ...
   Executá-lo de dentro de uma das máquinas virtuais na configuração do cluster.
...
2.  Defina uma porta de pesquisa. O número de porta de sonda de predefinido é **0**. No nosso exemplo, utilizamos a porta da sonda **62000**.
  ...
  Para definir um novo valor de ProbePort para o  **SAP <*SID*> IP * * recurso de cluster, execute o seguinte script do PowerShell para atualizar as variáveis de PowerShell para o seu ambiente:
  ...
  ```
  Depois do script é executado, será solicitado para reiniciar o grupo de cluster do SAP para ativar as alterações.
...
## <a name="094bc895-31d4-4471-91cc-1513b64e406a"></a>Alterar o tipo de início da instância de serviço do SAP ERS Windows
...
## <a name="18aa2b9d-92d2-4c0e-8ddd-5acaabda99e9"></a>Testar a ativação pós-falha de instância ASCS/SCS SAP e SIOS replicação
...
![Figura 6: Gestor de clusters de ativação pós-falha: grupo de cluster o SAP < SID > está em execução no nó de cluster A][sap-ha-guide-figure-5000]
...
4.  Reiniciar um nó de cluster utilizando o Azure PowerShell (esta ação inicia uma ativação pós-falha automática do SAP <*SID*> grupo de cluster do nó de um nó B).
  ...
  ![Figura 8: No Gestor de clusters de ativação pós-falha, o grupo de cluster do SAP < SID > está em execução no nó de cluster B][sap-ha-guide-figure-5002]
