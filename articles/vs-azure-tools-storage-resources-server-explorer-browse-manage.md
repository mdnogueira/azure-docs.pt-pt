...
Para ver os recursos da conta de emulador de armazenamento, expanda o **desenvolvimento** nós. Se o emulador do storage ainda não foi iniciado quando expande o **desenvolvimento** nós, irá iniciar automaticamente. Isto pode demorar vários segundos. Pode continuar a trabalhar nas outras áreas do Visual Studio enquanto inicia o emulador de armazenamento.

Para ver os recursos numa conta do storage, expanda o nó da conta de armazenamento no Explorador de servidores. Os subnodes seguintes são apresentadas:
...
2. No **criar contentor de Blob** caixa de diálogo, introduza o nome do novo contentor.  
3. Selecione Enter no teclado, ou pode clique ou toque em fora o campo de nome para guardar o contentor de blob.
...
* Elimine um blob.
  ...
  > [!NOTE]
  > Eliminar um ficheiro a partir de um contentor de blob não elimina o ficheiro subjacente. Apenas remove-lo do contentor do blob.
  > 
  > 
* Abra um blob.
...
2. No **carregar o novo ficheiro** caixa de diálogo, selecione o **procurar** botão para especificar o ficheiro que pretende carregar e, em seguida, introduza um nome de pasta o **pasta (opcional)** caixa.
...
3. Faça duplo clique na pasta ou selecione Enter para ver o conteúdo da pasta. Quando estiver na pasta do contentor, pode voltar à raiz do contentor, selecionando o **abrir o diretório principal** botão (seta).

### <a name="to-delete-a-container-folder"></a>Para eliminar uma pasta de contentor
* Elimine todos os ficheiros na pasta.
  
> [!NOTE]
> Uma vez pastas em contentores de BLOBs são pastas virtuais, não é possível criar uma pasta vazia. Também não é possível eliminar uma pasta para eliminar os conteúdos do ficheiro. Tem de eliminar todo o conteúdo de uma pasta para eliminar a pasta de si próprio.
> 
> 
...
* Selecione o **carregar Blob** botão quando o contentor está aberto para visualização na vista de contentor de blob.
...
O ficheiro é transferido para uma localização temporária e aberto no computador local. Tem de carregar o blob novamente depois de efetuar alterações.
...
2. No **Create Table** caixa de diálogo, introduza um nome para a tabela.
...
2. Abra o nó de conta de armazenamento que estão interessadas nas e, em seguida, abra o **tabelas** nó para ver uma lista de tabelas para a conta de armazenamento.
3. Abra o menu de atalho para uma tabela e, em seguida, selecione **tabela da vista**.
...
2. No **adicionar entidade** caixa de diálogo, introduza os valores a **PartitionKey** e **RowKey** propriedades.
...
2. Selecione o **construtor de consultas** botão na barra de ferramentas da tabela da vista.
...
3. Quando estiver pronto a construir a consulta, feche a caixa de diálogo. O formato de texto resultante da consulta é apresentado numa caixa de texto como um filtro de WCF Data Services.
...
2. No **criar conta de armazenamento** diálogo caixa, selecione ou introduza as seguintes informações:
...
Se tiver criado uma conta de armazenamento utilizando o **acesso de leitura Georreplicação redundante** opção de replicação, pode ver os respetivos pontos finais secundários:
...
> [!NOTE]
> Se eliminar uma conta do storage no Explorador de servidores, não afeta a conta de armazenamento ou de todos os dados que contém. Remove simplesmente a referência no Explorador de servidores. Para eliminar permanentemente uma conta de armazenamento, utilize o [portal clássico do Azure](http://go.microsoft.com/fwlink/?LinkID=213885).
> 
> 
