...
* **Gestão de registo de nome de anfitrião automática.** Juntamente com os registos DNS personalizados de alojamento, Azure manterá automaticamente registos de nome de anfitrião para as VMs nas redes virtuais especificadas.  Isto permite-lhe otimizar os nomes de domínio que sem necessitar de utilizar para criar soluções personalizadas de DNS ou modificar a aplicação.
...
* **Ferramentas familiares e experiência de utilizador.** Para reduzir a curva de aprendizagem, esta nova oferta utiliza as ferramentas de DNS do Azure (PowerShell, modelos do Resource Manager, REST API) já bem estabelecidas e será suporte adicionado para o portal e o CLI logo que possível.
...
Zonas DNS privada será gratuitamente durante a pré-visualização gerida. No anúncio de disponibilidade geral, esta funcionalidade irá utilizar um baseada na utilização modelo de preços semelhante para o DNS do Azure existente da oferta. 
