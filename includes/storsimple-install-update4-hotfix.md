...
5. Pesquise as correções adicionais listadas na tabela acima (**4011841**) e adicione-as ao cesto.
...
> [!NOTE]
> As correções têm de estar acessíveis a partir de ambos os controladores para detetar potenciais mensagens de erro a partir do elemento da rede > controlador.
>
> As correções têm de ser copiadas em três pastas separadas. A atualização de software do dispositivo tem de ser copiada na pasta _FirstOrderUpdate_, todas as outras atualizações não disruptivas devem ser copiadas na pasta _SecondOrderUpdate_ e as atualizações do modo de manutenção devem ser copiadas na pasta _ThirdOrderUpdate_.
...
3. Para instalar a correção, na linha de comandos, escreva:
    ...
    É apresentada abaixo uma saída de exemplo para instalar as primeiras atualizações de pedido.
...
7. Repita os passos 3 a 5 para instalar as atualizações de segunda ordem. Podem ser instaladas várias atualizações ao executar apenas o `Start-HcsHotfix cmdlet` e apontar para a pasta onde se encontram as atualizações de segunda ordem. O cmdlet irá executar todas as atualizações disponíveis na pasta. Se já estiver instalado uma atualização, a lógica de atualização irá detetar e não irá aplicar essa atualização. Depois de instalar todas as correções, utilize o cmdlet `Get-HcsSystem`. As versões devem ser:
