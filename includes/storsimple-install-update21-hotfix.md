...
3. Na caixa de pesquisa do catálogo Microsoft Update, introduza o número de Base de dados de conhecimento (KB) de correção que pretende transferir, por exemplo **3179904**e, em seguida, clique em **pesquisa**.
   
    A listagem de correção é apresentado, por exemplo, **cumulativa 2.2 atualização do pacote de Software para a série de 8000 do StorSimple**.
...
5. Procure as correções adicionais listados na tabela acima (**3103616**, **3146621**) e adicione cada o cesto.
...
4. Escreva **Y** quando lhe for pedido para confirmar a instalação da correção.
   ...
   > [!IMPORTANT]
   > Se instalar a atualização 2.2, instale apenas o ficheiro binário precedido pela 'all-hcsmdssoftwareudpate'. Não instale o Cis e a atualização de agente do MDS precedido pela all cismdsagentupdatebundle. Falha ao fazê-lo resultará num erro. 
...
1. Assim que a atualização do software for concluída, verifique as versões do software do sistema. Escreva:
   ...
   * `MdsAgentVersion: 30.0.4698.16` 
     ...
     > 
     > 
2. Repita os passos 3 a 5 para instalar as correções de modo de regular restantes.
   
   * A atualização de iSCSI KB3146621
   * A atualização WMI KB3103616
3. Ignore este passo se estão a atualizar a partir da atualização 2. Se estão a atualizar a partir de uma versão antes da atualização 2, também terá de transferir:

    - O controlador de LSI KB3121900
